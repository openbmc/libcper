...
static const int GEN_EXAMPLES = 1;
...
void cper_create_examples(const char *section_name){
    ...
    if (!outFile.is_open()) {
        std::cerr << "Failed to create/open CPER output file: " << cper_out << std::endl;
        return;
    }
	...
	if (jsonData.is_discarded()) {
		std::cerr << "cper_create_examples: JSON parse error:" << std::endl;
		return;
	}
...
void cper_example_section_ir_test(const char *section_name, int single_section, GEN_VALID_BITS_TEST_TYPE validBitsType)
{
	//Generate full CPER record for the given type.
	char *buf;
	size_t size;
	...
	nlohmann::json jsonData = nlohmann::json::parse(str, nullptr, false);
	if (jsonData.is_discarded()) {
		std::cerr << "cper_example_section_ir_test: JSON parse error:" << std::endl;
		fclose(record);
		free(buf);
		return;
	}
...
void cper_log_section_ir_test(const char *section_name, int single_section, GEN_VALID_BITS_TEST_TYPE validBitsType)
{
	//Generate full CPER record for the given type.
	char *buf;
	size_t size;
...
void cper_log_section_binary_test(const char *section_name, int single_section, GEN_VALID_BITS_TEST_TYPE validBitsType)
{
