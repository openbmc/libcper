...
// Tests a single randomly generated CPER section of the given type to ensure
// CPER-JSON IR validity.
void cper_log_section_ir_test(const char* section_name, int single_section)
{
    ...
    int valid = validate_schema_from_file(LIBCPER_JSON_SPEC, ir, error_message);
    json_object_put(ir);
    ASSERT_TRUE(valid) << "IR validation test failed (single section mode = "
                       << single_section << ") with message: " << error_message;
}

// Checks for binary round-trip equality for a given randomly generated CPER
// record.
...
// Tests randomly generated CPER sections for IR validity of a given type, in
// both single section mode and full CPER log mode.
...
// Tests randomly generated CPER sections for binary compatibility of a given
// type, in both single section mode and full CPER log mode.
...
TEST(CompileTimeAssertions, TwoWayConversion)
{
    ...
    {
        // If a conversion one way exists, a conversion the other way must
        // exist.
        std::string err =
        ...
        {
            ASSERT_NE(section_definitions[i].ToCPER, nullptr) << err;
        }
    }
}
...
TEST(CompileTimeAssertions, ShortcodeNoSpaces)
{
    ...
    {
        for (int j = 0; generator_definitions[i].ShortName[j + 1] != '\0'; j++)
        {
            ASSERT_FALSE(isspace(generator_definitions[i].ShortName[j]))
