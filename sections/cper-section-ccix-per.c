/**
 * Describes functions for converting CCIX PER log CPER sections from binary and
...
json_object* cper_section_ccix_per_to_ir(void* section)
{
    ...
    // CCIX PER Log.
    // This is formatted as described in Section 7.3.2 of CCIX Base
    ...
    if (remaining_length > 0)
    {
        ...
        char* encoded =
            base64_encode((UINT8*)cur_pos, remaining_length, &encoded_len);
        if (encoded == NULL)
        {
...
// Converts a single CCIX PER CPER-JSON section into CPER binary, outputting to
// the given stream.
