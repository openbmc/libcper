/**
 * Describes functions for converting IA32/x64 CPER sections from binary and
 *JSON format into an intermediate format.
 *
...
json_object* cper_ia32x64_register_32bit_to_ir(
    EFI_CONTEXT_IA32_REGISTER_STATE* registers);
json_object* cper_ia32x64_register_64bit_to_ir(
    EFI_CONTEXT_X64_REGISTER_STATE* registers);
...
// Converts the IA32/x64 error section described in the given descriptor into
// intermediate format.
json_object* cper_section_ia32x64_to_ir(void* section)
{
    ...
    json_object_object_add(validationBits, "localAPICIDValid",
                           json_object_new_boolean(record->ValidFields & 0x1));
    ...
    for (int i = 0; i < processor_error_info_num; i++)
    {
        json_object_array_add(
            error_info_array,
            cper_ia32x64_processor_error_info_to_ir(current_error_info));
        current_error_info++;
    }
    ...
    for (int i = 0; i < processor_context_info_num; i++)
    {
        ...
        // The context array is a non-fixed size, pointer is shifted within the
        // above function.
    }
    ...
    EFI_IA32_X64_PROCESS_ERROR_INFO* error_info)
{
    ...
    json_object_object_add(type, "guid", json_object_new_string(error_type));
    ...
    json_object_object_add(error_info_ir, "instructionPointer",
                           json_object_new_uint64(error_info->InstructionIP));

    return error_info_ir;
}

// Converts a single IA32/x64 cache or TLB check check info block into JSON IR
// format.
json_object* cper_ia32x64_cache_tlb_check_to_ir(
    EFI_IA32_X64_CACHE_CHECK_INFO* cache_tlb_check)
{
    ...
    json_object* transaction_type = integer_to_readable_pair(
        ...
        IA32X64_CHECK_INFO_TRANSACTION_TYPES_VALUES, "Unknown (Reserved)");
    json_object_object_add(cache_tlb_check_ir, "transactionType",
                           transaction_type);

    // Operation.
    json_object* operation = integer_to_readable_pair(
        cache_tlb_check->Operation, 9, IA32X64_CHECK_INFO_OPERATION_TYPES_KEYS,
        IA32X64_CHECK_INFO_OPERATION_TYPES_VALUES, "Unknown (Reserved)");
    ...
    json_object_object_add(cache_tlb_check_ir, "preciseIP",
    ...
    json_object_object_add(cache_tlb_check_ir, "overflow",
    ...
    cper_ia32x64_bus_check_to_ir(EFI_IA32_X64_BUS_CHECK_INFO* bus_check)
{
    ...
    json_object* validation = bitfield_to_ir(
        bus_check->ValidFields, 11, IA32X64_CHECK_INFO_VALID_BITFIELD_NAMES);
    ...
    // Operation.
    json_object* operation = integer_to_readable_pair(
        bus_check->Operation, 9, IA32X64_CHECK_INFO_OPERATION_TYPES_KEYS,
        IA32X64_CHECK_INFO_OPERATION_TYPES_VALUES, "Unknown (Reserved)");
    ...
    // Miscellaneous boolean fields.
    json_object_object_add(bus_check_ir, "processorContextCorrupt",
    ...
    json_object_object_add(bus_check_ir, "restartableIP",
...
json_object* cper_ia32x64_ms_check_to_ir(EFI_IA32_X64_MS_CHECK_INFO* ms_check)
{
    ...
    // Validation bits.
    json_object* validation =
        bitfield_to_ir(ms_check->ValidFields, 6,
    ...
    json_object_object_add(ms_check_ir, "processorContextCorrupt",
                           json_object_new_boolean(ms_check->ContextCorrupt));
    json_object_object_add(ms_check_ir, "uncorrected",
    ...
    json_object_object_add(ms_check_ir, "restartableIP",
    ...
    EFI_IA32_X64_PROCESSOR_CONTEXT_INFO* context_info, void** cur_pos)
{
    ...
    json_object* context_type = integer_to_readable_pair(
        context_info->RegisterType, 8, IA32X64_REGISTER_CONTEXT_TYPES_KEYS,
    ...
    json_object_object_add(context_info_ir, "msrAddress",
    ...
    {
        // No parseable data, just dump as base64 and shift the head to the next
        // item.
        *cur_pos = (void*)(context_info + 1);

        int32_t encoded_len = 0;
        char* encoded = base64_encode((UINT8*)*cur_pos, context_info->ArraySize,
                                      &encoded_len);
        if (encoded == NULL)
        {
...
// Converts a single IA32/x64 CPER-JSON section into CPER binary, outputting to
// the provided stream.
void ir_section_ia32x64_to_cper(json_object* section, FILE* out)
{
    ...
    int proc_error_info_num =
        json_object_get_int(
            json_object_object_get(validation, "processorErrorInfoNum")) &
        0x3F;
    int proc_ctx_info_num =
        json_object_get_int(
            json_object_object_get(validation, "processorContextInfoNum")) &
        0x3F;
    ...
    // Local APIC ID.
    section_cper->ApicId =
        json_object_get_uint64(json_object_object_get(section, "localAPICID"));
    ...
    cpuid_info_cper->Eax =
        json_object_get_uint64(json_object_object_get(cpuid_info, "eax"));
    cpuid_info_cper->Ebx =
        json_object_get_uint64(json_object_object_get(cpuid_info, "ebx"));
    cpuid_info_cper->Ecx =
        json_object_get_uint64(json_object_object_get(cpuid_info, "ecx"));
    cpuid_info_cper->Edx =
        json_object_get_uint64(json_object_object_get(cpuid_info, "edx"));
    ...
    {
        ir_ia32x64_error_info_to_cper(json_object_array_get_idx(error_info, i),
                                      out);
    }
...
// Converts a single CPER-JSON IA32/x64 error information structure into CPER
// binary, outputting to the provided stream.
void ir_ia32x64_error_info_to_cper(json_object* error_info, FILE* out)
{
    ...
    // Validation bits.
    error_info_cper->ValidFields =
        ir_to_bitfield(json_object_object_get(error_info, "validationBits"), 5,
                       IA32X64_PROCESSOR_ERROR_VALID_BITFIELD_NAMES);
                   ...
                   &gEfiIa32x64ErrorTypeTlbCheckGuid))
    {
        ir_ia32x64_cache_tlb_check_error_to_cper(
            check_info,
            (EFI_IA32_X64_CACHE_CHECK_INFO*)&error_info_cper->CheckInfo);
    }
                        ...
                        &gEfiIa32x64ErrorTypeBusCheckGuid))
    {
        ir_ia32x64_bus_check_error_to_cper(
            check_info,
            (EFI_IA32_X64_BUS_CHECK_INFO*)&error_info_cper->CheckInfo);
    }
    else if (guid_equal(&error_info_cper->ErrorType,
                        &gEfiIa32x64ErrorTypeMsCheckGuid))
    {
        ir_ia32x64_ms_check_error_to_cper(
            check_info,
            (EFI_IA32_X64_MS_CHECK_INFO*)&error_info_cper->CheckInfo);
    }
...
// Converts a single CPER-JSON IA32/x64 cache/TLB check error info structure to
// CPER binary.
void ir_ia32x64_cache_tlb_check_error_to_cper(
    json_object* check_info, EFI_IA32_X64_CACHE_CHECK_INFO* check_info_cper)
{
    // Validation bits.
    check_info_cper->ValidFields =
        ir_to_bitfield(json_object_object_get(check_info, "validationBits"), 8,
                       IA32X64_CHECK_INFO_VALID_BITFIELD_NAMES);
    ...
    check_info_cper->Level =
        json_object_get_uint64(json_object_object_get(check_info, "level"));
    ...
    check_info_cper->Overflow =
        json_object_get_boolean(json_object_object_get(check_info, "overflow"));
}
    ...
    json_object* check_info, EFI_IA32_X64_BUS_CHECK_INFO* check_info_cper)
{
    // Validation bits.
    check_info_cper->ValidFields =
        ir_to_bitfield(json_object_object_get(check_info, "validationBits"), 11,
                       IA32X64_CHECK_INFO_VALID_BITFIELD_NAMES);
    ...
    check_info_cper->Overflow =
        json_object_get_boolean(json_object_object_get(check_info, "overflow"));
    check_info_cper->TimeOut =
        json_object_get_boolean(json_object_object_get(check_info, "timedOut"));
}
    ...
    json_object* check_info, EFI_IA32_X64_MS_CHECK_INFO* check_info_cper)
{
    // Validation bits.
    check_info_cper->ValidFields =
        ir_to_bitfield(json_object_object_get(check_info, "validationBits"), 6,
                       IA32X64_CHECK_INFO_MS_CHECK_VALID_BITFIELD_NAMES);
...
// Converts a single CPER-JSON IA32/x64 context information structure into CPER
// binary, outputting to the provided stream.
void ir_ia32x64_context_info_to_cper(json_object* context_info, FILE* out)
{
    ...
    fwrite(context_info_cper, sizeof(EFI_IA32_X64_PROCESSOR_CONTEXT_INFO), 1,
           out);
    fflush(out);
...
// Converts a single CPER-JSON IA32 register array into CPER binary, outputting
// to the given stream.
void ir_ia32x64_ia32_registers_to_cper(json_object* registers, FILE* out)
{
    EFI_CONTEXT_IA32_REGISTER_STATE register_state;
    ...
    register_state.Cs =
        (UINT16)json_object_get_uint64(json_object_object_get(registers, "cs"));
    register_state.Ds =
        (UINT32)json_object_get_uint64(json_object_object_get(registers, "ds"));
    register_state.Ss =
        (UINT16)json_object_get_uint64(json_object_object_get(registers, "ss"));
    register_state.Es =
        (UINT16)json_object_get_uint64(json_object_object_get(registers, "es"));
    register_state.Fs =
        (UINT16)json_object_get_uint64(json_object_object_get(registers, "fs"));
    register_state.Gs =
        (UINT16)json_object_get_uint64(json_object_object_get(registers, "gs"));
    ...
    UINT64 gdtr =
        json_object_get_uint64(json_object_object_get(registers, "gdtr"));
    register_state.Gdtr[0] = gdtr & 0xFFFFFFFF;
    register_state.Gdtr[1] = gdtr >> 32;
    UINT64 idtr =
        json_object_get_uint64(json_object_object_get(registers, "idtr"));
    ...
    register_state.Tr =
        (UINT16)json_object_get_uint64(json_object_object_get(registers, "tr"));
...
// Converts a single CPER-JSON x64 register array into CPER binary, outputting
// to the given stream.
void ir_ia32x64_x64_registers_to_cper(json_object* registers, FILE* out)
{
    EFI_CONTEXT_X64_REGISTER_STATE register_state;
    register_state.Rax =
        json_object_get_uint64(json_object_object_get(registers, "rax"));
    register_state.Rbx =
        json_object_get_uint64(json_object_object_get(registers, "rbx"));
    register_state.Rcx =
        json_object_get_uint64(json_object_object_get(registers, "rcx"));
    register_state.Rdx =
        json_object_get_uint64(json_object_object_get(registers, "rdx"));
    register_state.Rsi =
        json_object_get_uint64(json_object_object_get(registers, "rsi"));
    register_state.Rdi =
        json_object_get_uint64(json_object_object_get(registers, "rdi"));
    register_state.Rbp =
        json_object_get_uint64(json_object_object_get(registers, "rbp"));
    register_state.Rsp =
        json_object_get_uint64(json_object_object_get(registers, "rsp"));
    register_state.R8 =
        json_object_get_uint64(json_object_object_get(registers, "r8"));
    register_state.R9 =
        json_object_get_uint64(json_object_object_get(registers, "r9"));
    register_state.R10 =
        json_object_get_uint64(json_object_object_get(registers, "r10"));
    register_state.R11 =
        json_object_get_uint64(json_object_object_get(registers, "r11"));
    register_state.R12 =
        json_object_get_uint64(json_object_object_get(registers, "r12"));
    register_state.R13 =
        json_object_get_uint64(json_object_object_get(registers, "r13"));
    register_state.R14 =
        json_object_get_uint64(json_object_object_get(registers, "r14"));
    register_state.R15 =
        json_object_get_uint64(json_object_object_get(registers, "r15"));
    register_state.Cs =
        (UINT16)json_object_get_int(json_object_object_get(registers, "cs"));
    register_state.Ds =
        (UINT16)json_object_get_int(json_object_object_get(registers, "ds"));
    register_state.Ss =
        (UINT16)json_object_get_int(json_object_object_get(registers, "ss"));
    register_state.Es =
        (UINT16)json_object_get_int(json_object_object_get(registers, "es"));
    register_state.Fs =
        (UINT16)json_object_get_int(json_object_object_get(registers, "fs"));
    register_state.Gs =
        (UINT16)json_object_get_int(json_object_object_get(registers, "gs"));
    register_state.Resv1 = 0;
    register_state.Rflags =
        json_object_get_uint64(json_object_object_get(registers, "rflags"));
    register_state.Rip =
        json_object_get_uint64(json_object_object_get(registers, "eip"));
    register_state.Cr0 =
        json_object_get_uint64(json_object_object_get(registers, "cr0"));
    register_state.Cr1 =
        json_object_get_uint64(json_object_object_get(registers, "cr1"));
    register_state.Cr2 =
        json_object_get_uint64(json_object_object_get(registers, "cr2"));
    register_state.Cr3 =
        json_object_get_uint64(json_object_object_get(registers, "cr3"));
    register_state.Cr4 =
        json_object_get_uint64(json_object_object_get(registers, "cr4"));
    register_state.Cr8 =
        json_object_get_uint64(json_object_object_get(registers, "cr8"));
    register_state.Gdtr[0] =
        json_object_get_uint64(json_object_object_get(registers, "gdtr_0"));
    register_state.Gdtr[1] =
        json_object_get_uint64(json_object_object_get(registers, "gdtr_1"));
    register_state.Idtr[0] =
        json_object_get_uint64(json_object_object_get(registers, "idtr_0"));
    register_state.Idtr[1] =
        json_object_get_uint64(json_object_object_get(registers, "idtr_1"));
    register_state.Ldtr =
        (UINT16)json_object_get_int(json_object_object_get(registers, "ldtr"));
    register_state.Tr =
        (UINT16)json_object_get_int(json_object_object_get(registers, "tr"));
