					...
					char **desc_string)
{
	...
	*desc_string = calloc(1, SECTION_DESC_STRING_SIZE);
	...
	add_int(record_ir, "processorContextInfoNum", processor_context_info_num);
	...
	EFI_IA32_X64_PROCESS_ERROR_INFO *error_info)
{
	...
	if (isvalid_prop_to_ir(&ui64Type, 1)) {
		add_uint(error_info_ir, "targetAddressID", error_info->TargetId);
	}
	...
	if (isvalid_prop_to_ir(&ui64Type, 4)) {
		add_uint(error_info_ir, "instructionPointer", error_info->InstructionIP);
	}
	...
	UINT32 *remaining_size)
{
	...
	*remaining_size -= sizeof(EFI_IA32_X64_PROCESSOR_CONTEXT_INFO);
	...
	add_uint(context_info_ir, "mmRegisterAddress", context_info->MmRegisterAddress);
...
cper_ia32x64_register_32bit_to_ir(EFI_CONTEXT_IA32_REGISTER_STATE *registers)
{
	...
	add_uint(ia32_registers, "gdtr", registers->Gdtr[0] +
				       ((UINT64)registers->Gdtr[1] << 32));
	add_uint(ia32_registers, "idtr", registers->Idtr[0] +
