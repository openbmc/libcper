...
{
	if (signature == NULL) {
		cper_print_log("Error: NULL signature\n");
		return -1;
	}
	int index = -1;
	// we're comparing i with section_handlers type
	size_t i = 0;
				       ...
				       char **desc_string)
{
	...
	*desc_string = calloc(1, SECTION_DESC_STRING_SIZE);
	...
	add_string(severity, "name", severity_name);
	int outstr_len = 0;
