/**
 * Describes functions for converting NVIDIA CPER sections from binary and JSON
...
json_object* cper_section_nvidia_to_ir(void* section)
{
    ...
    json_object_object_add(section_ir, "errorInstance",
    ...
    json_object_object_add(section_ir, "instanceBase",
    ...
    for (int i = 0; i < nvidia_error->NumberRegs; i++)
    {
        ...
        json_object_object_add(reg, "value", json_object_new_uint64(*++regPtr));
...
// Converts a single NVIDIA CPER-JSON section into CPER binary, outputting to
// the given stream.
void ir_section_nvidia_to_cper(json_object* section, FILE* out)
{
    ...
    // Signature.
    strncpy(
        section_cper->Signature,
        json_object_get_string(json_object_object_get(section, "signature")),
        sizeof(section_cper->Signature));

    // Fields.
    section_cper->ErrorType =
        json_object_get_int(json_object_object_get(section, "errorType"));
    section_cper->ErrorInstance =
        json_object_get_int(json_object_object_get(section, "errorInstance"));
    section_cper->Severity =
        json_object_get_int(json_object_object_get(section, "severity"));
    section_cper->Socket =
        json_object_get_int(json_object_object_get(section, "socket"));
    section_cper->NumberRegs =
        json_object_get_int(json_object_object_get(section, "numberRegs"));
    section_cper->InstanceBase =
        json_object_get_uint64(json_object_object_get(section, "instanceBase"));
    ...
    for (int i = 0; i < numRegs; i++)
    {
        ...
        *++regPtr =
            json_object_get_uint64(json_object_object_get(reg, "address"));
        *++regPtr =
            json_object_get_uint64(json_object_object_get(reg, "value"));
    }
