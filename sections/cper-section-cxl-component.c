/**
 * Describes functions for converting CXL component error CPER sections from
...
json_object* cper_section_cxl_component_to_ir(void* section)
{
    ...
    json_object* validation = bitfield_to_ir(
        cxl_error->ValidBits, 3, CXL_COMPONENT_ERROR_VALID_BITFIELD_NAMES);
    ...
    json_object_object_add(device_id, "busNumber",
    ...
    // The specification for this is defined within the CXL Specification
    // Section 8.2.9.1.
    ...
    if (remaining_len > 0)
    {
        ...
        char* encoded =
            base64_encode((UINT8*)cur_pos, remaining_len, &encoded_len);
        if (encoded == NULL)
        {
...
// Converts a single given CXL Component CPER-JSON section into CPER binary,
...
void ir_section_cxl_component_to_cper(json_object* section, FILE* out)
{
    ...
    section_cper->Length =
        json_object_get_uint64(json_object_object_get(section, "length"));

    // Validation bits.
    section_cper->ValidBits =
        ir_to_bitfield(json_object_object_get(section, "validationBits"), 3,
    ...
    section_cper->DeviceSerial =
        json_object_get_uint64(json_object_object_get(section, "deviceSerial"));
