					      ...
					      char **desc_string)
{
	...
	*desc_string = calloc(1, SECTION_DESC_STRING_SIZE);
	...
	if (isvalid_prop_to_ir(&ui64Type, 0)) {
		...
		add_int(device_id, "functionNumber", cxl_error->DeviceId.FunctionNumber);
		add_int(device_id, "deviceNumber", cxl_error->DeviceId.DeviceNumber);
		add_int(device_id, "busNumber", cxl_error->DeviceId.BusNumber);
		add_int(device_id, "segmentNumber", cxl_error->DeviceId.SegmentNumber);
		add_int(device_id, "slotNumber", cxl_error->DeviceId.SlotNumber);
