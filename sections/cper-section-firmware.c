/**
 * Describes functions for converting firmware CPER sections from binary and
...
json_object* cper_section_firmware_to_ir(void* section)
{
    ...
    json_object_object_add(section_ir, "recordID",
...
// Converts a single firmware CPER-JSON section into CPER binary, outputting to
// the given stream.
void ir_section_firmware_to_cper(json_object* section, FILE* out)
{
    ...
    section_cper->Revision =
        json_object_get_int(json_object_object_get(section, "revision"));
    section_cper->RecordId =
        json_object_get_uint64(json_object_object_get(section, "recordID"));
    string_to_guid(&section_cper->RecordIdGuid,
                   json_object_get_string(
                       json_object_object_get(section, "recordIDGUID")));
