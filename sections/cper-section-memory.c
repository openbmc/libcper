/**
 * Describes functions for converting memory error CPER sections from binary and
...
json_object* cper_section_platform_memory_to_ir(void* section)
{
    ...
    // Validation bitfield.
    json_object* validation = bitfield_to_ir(memory_error->ValidFields, 22,
    ...
    if ((memory_error->ValidFields >> 5) & 0x1)
    {
        ...
        json_object_object_add(bank, "value",
    ...
    else
    {
        ...
        json_object_object_add(
            bank, "address", json_object_new_uint64(memory_error->Bank & 0xFF));
        json_object_object_add(bank, "group",
    ...
    json_object_object_add(extended, "chipIdentification",
                           json_object_new_int(memory_error->Extended >> 5));
    ...
    json_object_object_add(section_ir, "moduleRank",
    ...
    json_object_object_add(section_ir, "bitPosition",
                           json_object_new_uint64(memory_error->BitPosition));
    json_object_object_add(section_ir, "requestorID",
    ...
    json_object_object_add(section_ir, "cardSmbiosHandle",
                           json_object_new_uint64(memory_error->CardHandle));
    json_object_object_add(section_ir, "moduleSmbiosHandle",
...
json_object* cper_section_platform_memory2_to_ir(void* section)
{
    ...
    json_object* validation = bitfield_to_ir(
        memory_error->ValidFields, 22, MEMORY_ERROR_2_VALID_BITFIELD_NAMES);
    json_object_object_add(section_ir, "validationBits", validation);
    ...
    json_object_object_add(
        ...
        json_object_new_string(
            (memory_error->Status & 0x1) == 0 ? "Corrected" : "Uncorrected"));
...
// Converts a single Memory Error IR section into CPER binary, outputting to the
// provided stream.
void ir_section_memory_to_cper(json_object* section, FILE* out)
{
    ...
    section_cper->ValidFields =
        ir_to_bitfield(json_object_object_get(section, "validationBits"), 22,
                       MEMORY_ERROR_VALID_BITFIELD_NAMES);

    // Error status.
    ir_generic_error_status_to_cper(
        json_object_object_get(section, "errorStatus"),
    ...
    section_cper->Extended |=
        json_object_get_boolean(json_object_object_get(extended, "rowBit16"));
    section_cper->Extended |=
        json_object_get_boolean(json_object_object_get(extended, "rowBit17"))
        << 1;
    section_cper->Extended |=
        json_object_get_int(
            json_object_object_get(extended, "chipIdentification"))
        << 5;
    ...
    section_cper->Node =
        (UINT16)json_object_get_uint64(json_object_object_get(section, "node"));
    section_cper->Card =
        (UINT16)json_object_get_uint64(json_object_object_get(section, "card"));
    ...
    section_cper->Row =
        (UINT16)json_object_get_uint64(json_object_object_get(section, "row"));
    ...
    section_cper->RequestorId =
        json_object_get_uint64(json_object_object_get(section, "requestorID"));
    section_cper->ResponderId =
        json_object_get_uint64(json_object_object_get(section, "responderID"));
    section_cper->TargetId =
        json_object_get_uint64(json_object_object_get(section, "targetID"));
...
// Converts a single Memory Error 2 IR section into CPER binary, outputting to
// the provided stream.
void ir_section_memory2_to_cper(json_object* section, FILE* out)
{
    ...
    section_cper->Device = (UINT32)json_object_get_uint64(
        json_object_object_get(section, "device"));
    section_cper->Row =
        (UINT32)json_object_get_uint64(json_object_object_get(section, "row"));
    section_cper->Column = (UINT32)json_object_get_uint64(
        json_object_object_get(section, "column"));
    section_cper->Rank =
        (UINT32)json_object_get_uint64(json_object_object_get(section, "rank"));
