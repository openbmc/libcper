/**
 * Describes functions for converting VT-d specific DMAr CPER sections from
...
json_object* cper_section_dmar_vtd_to_ir(void* section)
{
    ...
    json_object_object_add(fault_record_ir, "pasidPresent",
                           json_object_new_boolean(fault_record->PasidPresent));
    json_object_object_add(fault_record_ir, "faultReason",
                           json_object_new_uint64(fault_record->FaultReason));
    json_object_object_add(fault_record_ir, "pasidValue",
                           json_object_new_uint64(fault_record->PasidValue));
    json_object_object_add(fault_record_ir, "addressType",
    ...
    // Context entry.
    encoded_len = 0;
    encoded = base64_encode((UINT8*)vtd_error->ContextEntry, 16, &encoded_len);
    if (encoded == NULL)
    {
...
// Converts a single VT-d DMAR CPER-JSON segment into CPER binary, outputting to
// the given stream.
void ir_section_dmar_vtd_to_cper(json_object* section, FILE* out)
{
    ...
    // OEM ID.
    UINT64 oem_id =
        json_object_get_uint64(json_object_object_get(section, "oemID"));
    for (int i = 0; i < 6; i++)
    {
    ...
    section_cper->Version =
        (UINT8)json_object_get_int(json_object_object_get(section, "version"));
    section_cper->Revision =
        (UINT8)json_object_get_int(json_object_object_get(section, "revision"));
