					 ...
					 char **desc_string)
{
	...
	*desc_string = NULL;
	if (size < sizeof(EFI_DIRECTED_IO_DMAR_ERROR_DATA)) {
		cper_print_log("Error: VT-d DMAr section too small\n");
		return NULL;
	}
	...
	*desc_string = calloc(1, SECTION_DESC_STRING_SIZE);
	if (*desc_string == NULL) {
		cper_print_log(
			"Error: Failed to allocate VT-d DMAr desc string\n");
		return NULL;
	}
	...
	add_int(section_ir, "version", vtd_error->Version);
	add_int(section_ir, "revision", vtd_error->Revision);
	add_uint(section_ir, "oemID", oem_id);

	//Registers.
	add_uint(section_ir, "capabilityRegister", vtd_error->Capability);
	add_uint(section_ir, "extendedCapabilityRegister",
		 vtd_error->CapabilityEx);
	add_uint(section_ir, "globalCommandRegister", vtd_error->GlobalCommand);
	add_uint(section_ir, "globalStatusRegister", vtd_error->GlobalStatus);
	add_uint(section_ir, "faultStatusRegister", vtd_error->FaultStatus);
	...
	add_uint(fault_record_ir, "faultInformation",
		 fault_record->FaultInformation);
	add_uint(fault_record_ir, "sourceIdentifier",
		 fault_record->SourceIdentifier);
	add_bool(fault_record_ir, "privelegeModeRequested",
		 fault_record->PrivelegeModeRequested);
	add_bool(fault_record_ir, "executePermissionRequested",
		 fault_record->ExecutePermissionRequested);
	add_bool(fault_record_ir, "pasidPresent", fault_record->PasidPresent);
	add_uint(fault_record_ir, "faultReason", fault_record->FaultReason);
	add_uint(fault_record_ir, "pasidValue", fault_record->PasidValue);
	add_uint(fault_record_ir, "addressType", fault_record->AddressType);
	...
	//Root entry.
	add_binary_base64(section_ir, "rootEntry", (UINT8 *)vtd_error->RootEntry, sizeof(vtd_error->RootEntry));

	//Context entry.
	add_binary_base64(section_ir, "contextEntry", (UINT8 *)vtd_error->ContextEntry, sizeof(vtd_error->ContextEntry));

	//PTE entry for all page levels.
	add_uint(section_ir, "pageTableEntry_Level6", vtd_error->PteL6);
	add_uint(section_ir, "pageTableEntry_Level5", vtd_error->PteL5);
	add_uint(section_ir, "pageTableEntry_Level4", vtd_error->PteL4);
	add_uint(section_ir, "pageTableEntry_Level3", vtd_error->PteL3);
	add_uint(section_ir, "pageTableEntry_Level2", vtd_error->PteL2);
	add_uint(section_ir, "pageTableEntry_Level1", vtd_error->PteL1);
