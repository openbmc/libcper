				     ...
				     const EFI_ARM_RAS_NODE *node)
{
	...
	json_object_object_add(root, "componentType",
			       json_object_new_uint64(node->ComponentType));
	json_object_object_add(
		root, "errorSyndromeArrayOffset",
		json_object_new_uint64(node->ErrorSyndromeArrayOffset));
	json_object_object_add(
		root, "auxDataOffset",
		json_object_new_uint64(node->AuxiliaryDataOffset));
	json_object_object_add(
	...
	//	json_object_new_uint64(node->IPInstanceFormat));
	//json_object_object_add(root, "ipTypeFormat",
	//	json_object_new_uint64(node->IPTypeFormat));
	//
	//add_bytes_hex(root, "ipInstance", node->IPInstance,
	//	sizeof(node->IPInstance));
	//add_bytes_hex(root, "ipType", node->IPType, sizeof(node->IPType));
...
arm_ras_aux_emit_header_fields(const EFI_ARM_RAS_AUX_DATA_HEADER *auxHdr)
{
	...
	json_object_object_add(
		...
		json_object_new_uint64(auxHdr->AuxiliaryDataSize));
	json_object_object_add(
			   ...
			   const EFI_ARM_RAS_AUX_DATA_HEADER *auxHdr)
{
	...
	for (UINT16 ci = 0; ci < auxHdr->AddressSpaceArrayEntryCount; ci++) {
		...
		json_object_object_add(ctxObjInstance, "length",
				       json_object_new_uint64(ctx->Length));
		json_object_object_add(ctxObjInstance, "flags",
				       json_object_new_uint64(ctx->Flags));
		json_object_object_add(
					  ...
					  json_object *section)
{
	...
	if (json_object_object_get_ex(section, "ipInstance", &t)) {
		...
		if (hex_string_to_bytes(hex, strlen(hex), node->IPInstance,
					sizeof(node->IPInstance)) != 0) {
			cper_print_log(
				"Failed to convert IP instance hex string to bytes");
			return;
		}
	}
	if (json_object_object_get_ex(section, "ipType", &t)) {
		const char *hex = json_object_get_string(t);
		if (hex_string_to_bytes(hex, strlen(hex), node->IPType,
					sizeof(node->IPType)) != 0) {
			cper_print_log(
				"Failed to convert IP type hex string to bytes");
			return;
		}
	}
}
					...
					json_object *section)
{
	...
	if (json_object_object_get_ex(section, "userData", &t)) {
		...
		int len = cper_printable_string_length(s, strlen(s));
		if (len < 0 || (size_t)len > sizeof(node->UserData)) {
			cper_print_log("ARM RAS user data too long: %zu > %zu",
				       strlen(s), sizeof(node->UserData));
			return;
		}
