			      ...
			      json_object **root)
{
	*root = json_object_new_object();
	if (size < sizeof(EFI_ARM_RAS_NODE)) {
		...
		arm_ras_set_desc_string_invalid("invalid/too small",
						desc_string);
		return false;
	}
	...
	if (descBytes > (UINT64)(size - node->ErrorSyndromeArrayOffset)) {
		...
		arm_ras_set_desc_string_invalid("descriptor array out of range",
						desc_string);
		return false;
	}
	if (node->Revision != 1) {
		cper_print_log("Unsupported ARM RAS revision: %u",
			       node->Revision);
		arm_ras_set_desc_string_invalid("unsupported ARM RAS revision",
						desc_string);
		return false;
	}
	...
	if (node->ErrorSyndromeArrayOffset < sizeof(EFI_ARM_RAS_NODE) ||
		...
		arm_ras_set_desc_string_invalid("invalid ARM RAS offsets",
						desc_string);
		return false;
	}
				     ...
				     const EFI_ARM_RAS_NODE *node)
{
	...
	json_object *component_type = integer_to_readable_pair(
		node->ComponentType, ARM_RAS_COMPONENT_TYPE_COUNT,
		ARM_RAS_COMPONENT_TYPE_KEYS, ARM_RAS_COMPONENT_TYPE_VALUES,
		"Unknown");
	json_object_object_add(root, "componentType", component_type);
	...
	json_object *ip_instance_format = integer_to_readable_pair(
		node->IPInstanceFormat, ARM_RAS_IP_INSTANCE_FORMAT_COUNT,
		ARM_RAS_IP_INSTANCE_FORMAT_KEYS,
		ARM_RAS_IP_INSTANCE_FORMAT_VALUES, "Unknown");
	json_object_object_add(root, "ipInstanceFormat", ip_instance_format);
	json_object *ip_type_format = integer_to_readable_pair(
		node->IPTypeFormat, ARM_RAS_IP_TYPE_FORMAT_COUNT,
		ARM_RAS_IP_TYPE_FORMAT_KEYS, ARM_RAS_IP_TYPE_FORMAT_VALUES,
		"Unknown");
	json_object_object_add(root, "ipTypeFormat", ip_type_format);

	add_bytes_hex(root, "ipInstanceHex", node->IPInstance,
		      sizeof(node->IPInstance));
	add_bytes_hex(root, "ipTypeHex", node->IPType, sizeof(node->IPType));

	add_untrusted_string(root, "userData", node->UserData,
				  ...
				  UINT32 auxLen)
{
	...
	*/
	...
	if (auxHdr->AuxiliaryDataSize > auxLen) {
		return false;
	}
	if (auxHdr->AuxiliaryDataSize < sizeof(EFI_ARM_RAS_AUX_DATA_HEADER)) {
		return false;
	}
	if (auxHdr->KeyValuePairArrayOffset <
			   ...
			   const EFI_ARM_RAS_AUX_DATA_HEADER *auxHdr)
{
	...
	for (UINT16 ci = 0; ci < auxHdr->AddressSpaceArrayEntryCount; ci++) {
		...
		const UINT8 *reg_base =
			cursor + sizeof(EFI_ARM_RAS_AUX_CONTEXT_HEADER);
		const EFI_ARM_RAS_AUX_MM_REG_ENTRY *regArr =
			(const EFI_ARM_RAS_AUX_MM_REG_ENTRY *)reg_base;
					   ...
					   json_object *section)
{
	...
	if (json_object_object_get_ex(section, "componentType", &obj)) {
		node->ComponentType = (UINT8)readable_pair_to_integer(obj);
	}
	if (json_object_object_get_ex(section, "ipInstanceFormat", &obj)) {
		node->IPInstanceFormat = (UINT8)readable_pair_to_integer(obj);
	}
	if (json_object_object_get_ex(section, "ipTypeFormat", &obj)) {
		node->IPTypeFormat = (UINT8)readable_pair_to_integer(obj);
	}
}
...
{
	json_object *t = NULL;
	if (json_object_object_get_ex(section, "ipInstanceHex", &t)) {
		const char *hex = json_object_get_string(t);
		for (int i = 0; i < (int)sizeof(node->IPInstance) &&
				(int)strlen(hex) >= 2 * (i + 1);
		     i++) {
			unsigned v;
	...
	if (json_object_object_get_ex(section, "ipTypeHex", &t)) {
		const char *hex = json_object_get_string(t);
		for (int i = 0; i < (int)sizeof(node->IPType) &&
				(int)strlen(hex) >= 2 * (i + 1);
		     i++) {
			unsigned v;
			sscanf(&hex[i * 2], "%02x", &v);
					...
					json_object *section)
{
	...
	if (json_object_object_get_ex(section, "userData", &t)) {
		...
		size_t l = strlen(s) > (int)sizeof(node->UserData) ?
				   (int)sizeof(node->UserData) - 1 :
				   strlen(s);
