				     ...
				     const EFI_ARM_RAS_NODE *node)
{
	...
	case 2:
		...
		add_int_hex_8(root, "pidr1", node->IPType.pidr.PIDR1);
					   ...
					   json_object *section)
{
	...
	if (json_object_object_get_ex(section, "ipInstanceFormat", &obj)) {
		json_object *ipInstanceFormatValue = NULL;
		if (json_object_object_get_ex(obj, "value",
					      &ipInstanceFormatValue)) {
			node->IPInstanceFormat = (UINT8)json_object_get_int(
				ipInstanceFormatValue);
		}
	}
	if (json_object_object_get_ex(section, "ipInstance", &obj)) {
		...
		get_value_hex_64(
			...
			&node->IPInstance.localAddressIdentifier.BaseAddress);
		// Not supported yet
		//get_value_hex_64(ipInstance, "socSpecificIpIdentifier", &node->IPInstance.socSpecificIpIdentifier.SocSpecificIPIdentifier);
	}
	if (json_object_object_get_ex(section, "ipTypeFormat", &obj)) {
		get_value_hex_64(obj, "midrEL",
				 &node->IPType.smmuIidr.MIDR_EL1);
					...
					json_object *section)
{
	...
	if (json_object_object_get_ex(section, "userData", &t)) {
		...
		if (len < 0) {
			cper_print_log("ARM RAS user data invalid");
			return;
		}
