...
static void arm_ras_set_desc_string_valid(const EFI_ARM_RAS_NODE *node,
		...
		*desc_string = malloc(SECTION_DESC_STRING_SIZE);
		if (*desc_string) {
			snprintf(*desc_string, SECTION_DESC_STRING_SIZE,
				 "ARM RAS (rev %u) legacyType %u, %u entries",
				 node->Revision, node->ComponentType,
				 descriptorCount);
		}
	}
}
...
static void arm_ras_bytes_to_hex(char *out, size_t out_len, const UINT8 *in,
				 size_t in_len)
{
	if (!out || out_len == 0) {
		return;
	...
	if (out_len < (in_len * 2 + 1)) {
		/* Callers should size correctly; fail safe with empty string. */
		out[0] = 0;
		return;
	}
				     ...
				     const EFI_ARM_RAS_NODE *node)
{
	...
	char hexBuf[ARM_RAS_IP_TYPE_HEX_LEN + 1];
	arm_ras_bytes_to_hex(hexBuf, sizeof(hexBuf), node->IPInstance,
			     ARM_RAS_IP_INSTANCE_BYTES);
	json_object_object_add(root, "ipInstance",
			       json_object_new_string_len(
				       hexBuf, ARM_RAS_IP_INSTANCE_HEX_LEN));
	arm_ras_bytes_to_hex(hexBuf, sizeof(hexBuf), node->IPType,
			     ARM_RAS_IP_TYPE_BYTES);
	json_object_object_add(
		root, "ipType",
		json_object_new_string_len(hexBuf, ARM_RAS_IP_TYPE_HEX_LEN));
					      ...
					      UINT32 descriptorCount)
{
	...
	for (UINT32 i = 0; i < descriptorCount; i++) {
		...
		json_object_object_add(
			desc, "rasExtensionRevision",
			json_object_new_uint64(d.RasExtensionRevision));
		json_object_object_add(desc, "errFR",
				       json_object_new_uint64(d.ERR_FR));
		json_object_object_add(desc, "errCTLR",
				       json_object_new_uint64(d.ERR_CTLR));
		json_object_object_add(desc, "errSTATUS",
				       json_object_new_uint64(d.ERR_STATUS));
		json_object_object_add(desc, "errADDR",
				       json_object_new_uint64(d.ERR_ADDR));
		json_object_object_add(desc, "errMISC0",
				       json_object_new_uint64(d.ERR_MISC0));
		json_object_object_add(desc, "errMISC1",
					...
					char **desc_string)
{
	...
	if (descBytes > (UINT64)(size - node.ErrorSyndromeArrayOffset)) {
		cper_print_log(
			"ARM RAS descriptor array out of range - omitting descriptors");
		descriptorCount = 0;
	}

	arm_ras_set_desc_string_valid(&node, descriptorCount, desc_string);
					     ...
					     UINT32 *afterDescriptors)
{
	*descArr = NULL;
	*descCount = 0;
	if (json_object_object_get_ex(section, "errorSyndromeArray", descArr)) {
				   ...
				   json_object *kvpArr, UINT32 kvpOffset)
{
	...
	for (UINT16 ki = 0; ki < kvpCount; ki++) {
		...
		if (hexKey && strlen(hexKey) >= ARM_RAS_KEY_HEX_LEN) {
			...
			memset(kvOut[ki].Key, 0, ARM_RAS_IP_INSTANCE_BYTES);
		}
