/**
 * Describes functions for converting IOMMU specific DMAr CPER sections from
...
json_object* cper_section_dmar_iommu_to_ir(void* section)
{
    ...
    char* encoded =
        base64_encode((UINT8*)iommu_error->EventLogEntry, 16, &encoded_len);
    if (encoded == NULL)
    {
    ...
    encoded =
        base64_encode((UINT8*)iommu_error->DeviceTableEntry, 32, &encoded_len);
    if (encoded == NULL)
    {
        printf("Failed to allocate encode output buffer. \n");
        return NULL;
...
// Converts a single DMAR IOMMU CPER-JSON section into CPER binary, outputting
// to the given stream.
void ir_section_dmar_iommu_to_cper(json_object* section, FILE* out)
{
    EFI_IOMMU_DMAR_ERROR_DATA* section_cper = (EFI_IOMMU_DMAR_ERROR_DATA*)
        calloc(1, sizeof(EFI_IOMMU_DMAR_ERROR_DATA));
