/**
 * Describes functions for converting PCI/PCI-X bus CPER sections from binary
...
json_object* cper_section_pci_bus_to_ir(void* section)
{
    ...
    UINT8 command_type = (bus_error->BusCommand >> 56) & 0x1; // Byte 7, bit 0.
...
// Converts a single provided PCI/PCI-X bus CPER-JSON section into CPER binary,
// outputting to the provided stream.
void ir_section_pci_bus_to_cper(json_object* section, FILE* out)
{
    EFI_PCI_PCIX_BUS_ERROR_DATA* section_cper = (EFI_PCI_PCIX_BUS_ERROR_DATA*)
        calloc(1, sizeof(EFI_PCI_PCIX_BUS_ERROR_DATA));

    // Validation bits.
    section_cper->ValidFields =
        ir_to_bitfield(json_object_object_get(section, "validationBits"), 9,
    ...
    UINT16 bus_number =
        (UINT8)json_object_get_int(json_object_object_get(bus_id, "busNumber"));
    ...
    section_cper->BusAddress =
        json_object_get_uint64(json_object_object_get(section, "busAddress"));
    section_cper->BusData =
        json_object_get_uint64(json_object_object_get(section, "busData"));
