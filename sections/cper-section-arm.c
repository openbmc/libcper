/**
 * Describes functions for converting ARM CPER sections from binary and JSON
 *format into an intermediate format.
 *
...
json_object* cper_arm_processor_context_to_ir(
    EFI_ARM_CONTEXT_INFORMATION_HEADER* header, void** cur_pos);
json_object* cper_arm_cache_tlb_error_to_ir(
    EFI_ARM_CACHE_ERROR_STRUCTURE* cache_tlb_error,
...
void ir_arm_error_bus_info_to_cper(
    json_object* error_information,
...
json_object* cper_section_arm_to_ir(void* section)
{
    ...
    json_object* validation =
        bitfield_to_ir(record->ValidFields, 4, ARM_ERROR_VALID_BITFIELD_NAMES);
    ...
    json_object_object_add(
        ...
        json_object_new_string(
            record->ErrorAffinityLevel < 4 ? "Vendor Defined" : "Reserved"));
    ...
    if (!(record->RunningState >> 31))
    {
        ...
        // This can't be made human readable, as it is unknown whether this will
        // be the pre-PSCI 1.0 format or the newer Extended StateID format.
        json_object_object_add(section_ir, "psciState",
    ...
    // The current position is moved within the processing, as it is a dynamic
    // size structure.
    ...
    for (int i = 0; i < record->ContextInfoNum; i++)
    {
        ...
        json_object* processor_context =
            cper_arm_processor_context_to_ir(header, (void**)&cur_pos);
    ...
    if (cur_pos < (uint8_t*)section + record->SectionLength)
    {
        ...
        size_t input_size = (uint8_t*)section + record->SectionLength - cur_pos;
        int32_t encoded_len = 0;
        char* encoded = base64_encode(cur_pos, input_size, &encoded_len);
        if (encoded == NULL)
        {
            return NULL;
        }
    ...
    cper_arm_error_info_to_ir(EFI_ARM_ERROR_INFORMATION_ENTRY* error_info)
{
    ...
    // Flags.
    json_object* flags =
        bitfield_to_ir(error_info->Flags, 4, ARM_ERROR_INFO_ENTRY_FLAGS_NAMES);
    ...
    switch (error_info->Type)
    {
        ...
        case ARM_ERROR_INFORMATION_TYPE_TLB:   // TLB
            error_subinfo = cper_arm_cache_tlb_error_to_ir(
                (EFI_ARM_CACHE_ERROR_STRUCTURE*)&error_info->ErrorInformation,
                error_info);
            break;
        case ARM_ERROR_INFORMATION_TYPE_BUS: // Bus
            error_subinfo = cper_arm_bus_error_to_ir(
                (EFI_ARM_BUS_ERROR_STRUCTURE*)&error_info->ErrorInformation);
            break;

        default:
            // Unknown/microarch, so can't be made readable. Simply dump as a
            // uint64 data object.
            error_subinfo = json_object_new_object();
            json_object_object_add(
                error_subinfo, "data",
                json_object_new_uint64(error_info->ErrorInformation.Value));
            break;
    }
...
// Converts a single ARM cache/TLB error information structure into JSON IR
// format.
json_object* cper_arm_cache_tlb_error_to_ir(
    EFI_ARM_CACHE_ERROR_STRUCTURE* cache_tlb_error,
    EFI_ARM_ERROR_INFORMATION_ENTRY* error_info)
{
    ...
    json_object* transaction_type = integer_to_readable_pair(
        cache_tlb_error->TransactionType, 3, ARM_ERROR_TRANSACTION_TYPES_KEYS,
    ...
    if (error_info->Type == 0)
    {
        ...
        operation = integer_to_readable_pair(
            cache_tlb_error->Operation, 11, ARM_CACHE_BUS_OPERATION_TYPES_KEYS,
    ...
    else
    {
        // TLB operation.
        operation = integer_to_readable_pair(
            cache_tlb_error->Operation, 9, ARM_TLB_OPERATION_TYPES_KEYS,
    ...
    json_object_object_add(
        ...
        json_object_new_boolean(cache_tlb_error->ProcessorContextCorrupt));
    json_object_object_add(cache_tlb_error_ir, "corrected",
                           json_object_new_boolean(cache_tlb_error->Corrected));
    json_object_object_add(cache_tlb_error_ir, "precisePC",
...
json_object* cper_arm_bus_error_to_ir(EFI_ARM_BUS_ERROR_STRUCTURE* bus_error)
{
    ...
    json_object* validation = bitfield_to_ir(
        bus_error->ValidationBits, 12, ARM_BUS_ERROR_VALID_BITFIELD_NAMES);
    ...
    json_object_object_add(bus_error_ir, "restartablePC",
    ...
    json_object* participation_type = integer_to_readable_pair(
        bus_error->ParticipationType, 4, ARM_BUS_PARTICIPATION_TYPES_KEYS,
    ...
    json_object_object_add(
        access_mode, "name",
        json_object_new_string(
            bus_error->AccessMode == 0 ? "Secure" : "Normal"));
    ...
    EFI_ARM_CONTEXT_INFORMATION_HEADER* header, void** cur_pos)
{
    ...
    json_object* context_type = integer_to_readable_pair(
        ...
        ARM_PROCESSOR_INFO_REGISTER_CONTEXT_TYPES_VALUES, "Unknown (Reserved)");
    json_object_object_add(context_ir, "registerContextType", context_type);

    // Register array size (bytes).
    json_object_object_add(context_ir, "registerArraySize",
    ...
    *cur_pos = (void*)(header + 1);
    ...
    switch (header->RegisterContextType)
    {
        ...
        case EFI_ARM_CONTEXT_TYPE_AARCH32_EL1:
            register_array = uniform_struct_to_ir(
                (UINT32*)cur_pos,
                sizeof(EFI_ARM_AARCH32_EL1_CONTEXT_REGISTERS) / sizeof(UINT32),
                ARM_AARCH32_EL1_REGISTER_NAMES);
            break;
        case EFI_ARM_CONTEXT_TYPE_AARCH32_EL2:
            register_array = uniform_struct_to_ir(
                (UINT32*)cur_pos,
                sizeof(EFI_ARM_AARCH32_EL2_CONTEXT_REGISTERS) / sizeof(UINT32),
        ...
        case EFI_ARM_CONTEXT_TYPE_AARCH64_EL1:
            register_array = uniform_struct64_to_ir(
                (UINT64*)cur_pos,
                sizeof(EFI_ARM_AARCH64_EL1_CONTEXT_REGISTERS) / sizeof(UINT64),
                ARM_AARCH64_EL1_REGISTER_NAMES);
            break;
        case EFI_ARM_CONTEXT_TYPE_AARCH64_EL2:
            register_array = uniform_struct64_to_ir(
                (UINT64*)cur_pos,
                sizeof(EFI_ARM_AARCH64_EL2_CONTEXT_REGISTERS) / sizeof(UINT64),
                ARM_AARCH64_EL2_REGISTER_NAMES);
            break;
        case EFI_ARM_CONTEXT_TYPE_AARCH64_EL3:
            register_array = uniform_struct64_to_ir(
                (UINT64*)cur_pos,
                sizeof(EFI_ARM_AARCH64_EL3_CONTEXT_REGISTERS) / sizeof(UINT64),
        ...
        default:
            ...
            char* encoded = base64_encode(
                (UINT8*)cur_pos, header->RegisterArraySize, &encoded_len);
            if (encoded == NULL)
            {
...
// Converts a single CPER-JSON ARM error section into CPER binary, outputting to
// the given stream.
void ir_section_arm_to_cper(json_object* section, FILE* out)
{
    ...
    section_cper->ValidFields =
        ir_to_bitfield(json_object_object_get(section, "validationBits"), 4,
                       ARM_ERROR_VALID_BITFIELD_NAMES);

    // Count of error/context info structures.
    section_cper->ErrInfoNum =
        json_object_get_int(json_object_object_get(section, "errorInfoNum"));
    section_cper->ContextInfoNum =
        json_object_get_int(json_object_object_get(section, "contextInfoNum"));
    ...
    section_cper->MPIDR_EL1 =
        json_object_get_uint64(json_object_object_get(section, "mpidrEl1"));
    section_cper->MIDR_EL1 =
        json_object_get_uint64(json_object_object_get(section, "midrEl1"));
    section_cper->RunningState =
        json_object_get_boolean(json_object_object_get(section, "running"));
    ...
    {
        ir_arm_error_info_to_cper(json_object_array_get_idx(error_info, i),
                                  out);
    }
    ...
    {
        ir_arm_context_info_to_cper(json_object_array_get_idx(context_info, i),
                                    out);
    }
    ...
    if (vendor_specific_info != NULL)
    {
        ...
        UINT8* decoded =
            base64_decode(json_object_get_string(vendor_info_string),
...
// Converts a single ARM error information structure into CPER binary,
// outputting to the given stream.
void ir_arm_error_info_to_cper(json_object* error_info, FILE* out)
{
    EFI_ARM_ERROR_INFORMATION_ENTRY error_info_cper;

    // Version, length.
    error_info_cper.Version =
        json_object_get_int(json_object_object_get(error_info, "version"));
    error_info_cper.Length =
        json_object_get_int(json_object_object_get(error_info, "length"));

    // Validation bits.
    error_info_cper.ValidationBits =
    ...
    // Flags object.
    error_info_cper.Flags =
        (UINT8)ir_to_bitfield(json_object_object_get(error_info, "flags"), 4,
...
// Converts a single ARM cache/TLB error information structure into a CPER
// structure.
void ir_arm_error_cache_tlb_info_to_cper(
    json_object* error_information,
    EFI_ARM_CACHE_ERROR_STRUCTURE* error_info_cper)
{
    ...
    error_info_cper->ProcessorContextCorrupt = json_object_get_boolean(
        json_object_object_get(error_information, "processorContextCorrupt"));
...
// Converts a single ARM context information structure into CPER binary,
// outputting to the given stream.
void ir_arm_context_info_to_cper(json_object* context_info, FILE* out)
{
    ...
    info_header.Version =
        json_object_get_int(json_object_object_get(context_info, "version"));
...
// Converts a single AARCH32 GPR CPER-JSON object to CPER binary, outputting to
// the given stream.
void ir_arm_aarch32_gpr_to_cper(json_object* registers, FILE* out)
{
...
// Converts a single AARCH32 EL1 register set CPER-JSON object to CPER binary,
// outputting to the given stream.
void ir_arm_aarch32_el1_to_cper(json_object* registers, FILE* out)
{
    // Get uniform register array.
...
// Converts a single AARCH32 EL2 register set CPER-JSON object to CPER binary,
// outputting to the given stream.
void ir_arm_aarch32_el2_to_cper(json_object* registers, FILE* out)
{
    // Get uniform register array.
...
// Converts a single AARCH32 secure register set CPER-JSON object to CPER
// binary, outputting to the given stream.
void ir_arm_aarch32_secure_to_cper(json_object* registers, FILE* out)
{
    // Get uniform register array.
    ...
    ir_to_uniform_struct(
        registers, (UINT32*)&reg_array,
        sizeof(EFI_ARM_AARCH32_SECURE_CONTEXT_REGISTERS) / sizeof(UINT32),
        ARM_AARCH32_SECURE_REGISTER_NAMES);
...
// Converts a single AARCH64 GPR CPER-JSON object to CPER binary, outputting to
// the given stream.
void ir_arm_aarch64_gpr_to_cper(json_object* registers, FILE* out)
{
    // Get uniform register array.
...
// Converts a single AARCH64 EL1 register set CPER-JSON object to CPER binary,
// outputting to the given stream.
void ir_arm_aarch64_el1_to_cper(json_object* registers, FILE* out)
{
    // Get uniform register array.
...
// Converts a single AARCH64 EL2 register set CPER-JSON object to CPER binary,
// outputting to the given stream.
void ir_arm_aarch64_el2_to_cper(json_object* registers, FILE* out)
{
    // Get uniform register array.
...
// Converts a single AARCH64 EL3 register set CPER-JSON object to CPER binary,
// outputting to the given stream.
void ir_arm_aarch64_el3_to_cper(json_object* registers, FILE* out)
{
    // Get uniform register array.
...
// Converts a single ARM miscellaneous register set CPER-JSON object to CPER
// binary, outputting to the given stream.
void ir_arm_misc_registers_to_cper(json_object* registers, FILE* out)
{
    ...
    reg_array.MrsOp2 =
        json_object_get_uint64(json_object_object_get(mrs_encoding, "op2"));
    reg_array.MrsCrm =
        json_object_get_uint64(json_object_object_get(mrs_encoding, "crm"));
    reg_array.MrsCrn =
        json_object_get_uint64(json_object_object_get(mrs_encoding, "crn"));
    reg_array.MrsOp1 =
        json_object_get_uint64(json_object_object_get(mrs_encoding, "op1"));
    reg_array.MrsO0 =
        json_object_get_uint64(json_object_object_get(mrs_encoding, "o0"));

    // Actual register value.
    reg_array.Value =
        json_object_get_uint64(json_object_object_get(registers, "value"));
...
// Converts a single ARM unknown register CPER-JSON object to CPER binary,
// outputting to the given stream.
void ir_arm_unknown_register_to_cper(json_object* registers, FILE* out)
{
