...
#define CXL_PROTOCOL_ERROR_VALID_BITFIELD_NAMES                                \
    ...
    {                                                                          \
        "cxlAgentTypeValid", "cxlAgentAddressValid", "deviceIDValid",          \
            "deviceSerialValid", "capabilityStructureValid", "cxlDVSECValid",  \
            "cxlErrorLogValid"                                                 \
    }
...
typedef union
{
    ...
    UINT64 PortRcrbBaseAddress; // Active when the agent is a CXL1.1 host
                                // downstream port in CxlAgentType.
