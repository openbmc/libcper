				     ...
				     char **desc_string)
{
	...
	*desc_string = calloc(1, SECTION_DESC_STRING_SIZE);
	...
	if (isvalid_prop_to_ir(&ui64Type, 1)) {
		...
		add_int(version, "minor", bcd_to_int(
					       pcie_error->Version & 0xFF));
		add_int(version, "major", bcd_to_int(
	...
	if (isvalid_prop_to_ir(&ui64Type, 2)) {
		...
		add_uint(command_status, "commandRegister", pcie_error->CommandStatus &
					       0xFFFF);
		add_uint(command_status, "statusRegister", pcie_error->CommandStatus >>
					       16);
	...
	if (isvalid_prop_to_ir(&ui64Type, 4)) {
		add_uint(section_ir, "deviceSerialNumber", pcie_error->SerialNo);
	}

	//Bridge control status.
	if (isvalid_prop_to_ir(&ui64Type, 5)) {
		json_object *bridge_control_status = json_object_new_object();
		add_uint(bridge_control_status, "secondaryStatusRegister", pcie_error->BridgeControlStatus &
					       0xFFFF);
		add_uint(bridge_control_status, "controlRegister", pcie_error->BridgeControlStatus >> 16);
