/**
 * Describes functions for converting PCIe CPER sections from binary and JSON
...
json_object* cper_section_pcie_to_ir(void* section)
{
    ...
    // Validation bits.
    json_object* validation = bitfield_to_ir(pcie_error->ValidFields, 8,
    ...
    json_object_object_add(
        ...
        json_object_new_uint64(pcie_error->DevBridge.PrimaryOrDeviceBus));
    json_object_object_add(
    ...
    // Capability Structure (36-byte, padded to 60 bytes) or PCIe 2.0 Capability
    // Structure (60-byte). There does not seem to be a way to differentiate
    // these, so this is left as a b64 dump.
    int32_t encoded_len = 0;

    char* encoded =
        base64_encode((UINT8*)pcie_error->Capability.PcieCap, 60, &encoded_len);
    if (encoded == NULL)
    {
    ...
    encoded =
        base64_encode((UINT8*)pcie_error->AerInfo.PcieAer, 96, &encoded_len);
    if (encoded == NULL)
    {
        printf("Failed to allocate encode output buffer. \n");
    }
...
// Converts a single CPER-JSON PCIe section into CPER binary, outputting to the
// given stream.
void ir_section_pcie_to_cper(json_object* section, FILE* out)
{
    ...
    section_cper->ValidFields =
        ir_to_bitfield(json_object_object_get(section, "validationBits"), 8,
                       PCIE_ERROR_VALID_BITFIELD_NAMES);
    ...
    section_cper->DevBridge.PrimaryOrDeviceBus = (UINT8)json_object_get_uint64(
        json_object_object_get(device_id, "primaryOrDeviceBusNumber"));
    ...
    UINT32 bridge_status = (UINT16)json_object_get_uint64(
        json_object_object_get(bridge_control, "secondaryStatusRegister"));
