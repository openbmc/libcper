...
#define JSON_FIELD_HEX(field, ir, decode) \
	JSON_FIELD(field, ir, decode) \
	snprintf(hexstring_buf, EFI_UINT64_HEX_STRING_LEN, \
		"0x%08" PRIX32, decode->field); \
	json_object_object_add( \
		ir, #field "_hex", \
		json_object_new_string(hexstring_buf));

#define CAP_FIELD(field) JSON_FIELD_HEX(field, pcie_capability_ir, cap_decode)
#define AER_FIELD(field) JSON_FIELD_HEX(field, aer_capability_ir, aer_decode)
...
struct link_status_2 {
	UINT32 tbd1:10; // bits [9:0] for future extensions
	UINT32 flit_mode_status:1; // bit [10]
	UINT32 tbd2:5; // bits [15:11] for future extensions
};
...
struct capability_registers {
	...
	union {
		struct link_status_2 link_status_2_fields;
		UINT32 link_status_2;
	};
...
#define TYPE_MR       0b00000
#define TYPE_IO       0b00010
#define TYPE_CFG0     0b00100
#define TYPE_CFG1     0b00101
#define TYPE_DMRW     0b11011
#define TYPE_CPL      0b01010
#define TYPE_CPLLK    0b01011
#define TYPE_FETCHADD 0b01100
#define TYPE_SWAP     0b01101
#define TYPE_CAS      0b01110
#define TYPE_MSG0     0b10000
#define TYPE_MSG1     0b10001
#define TYPE_MSG2     0b10010
#define TYPE_MSG3     0b10011
#define TYPE_MSG4     0b10100
#define TYPE_MSG5     0b10101
...
json_object *parse_tlp_header_log(UINT32 *tlp_header_log)
{
	...
	switch(req_tlp_header->fmt) {
		case FMT_3DWND: fmt_str = "3DW (no data)"; break;
		case FMT_4DWND: fmt_str = "4DW (no data)"; break;
		case FMT_3DWD: fmt_str = "3DW (with data)"; break;
		case FMT_4DWD: fmt_str = "4DW (with data)"; break;
		default: fmt_str = "Reserved"; break;
	}
...
#define TYPE_RESERVED "TLP Prefix", "Reserved", "Reserved", "Reserved"
	struct type_decode_entry {
		...
		{TYPE_MR,       {"MRd",      "MRd",      "MWr",      "MWr",
			TYPE_RESERVED}},
		{TYPE_IO,       {"IORd",     "Reserved", "IOWr",     "Reserved",
			TYPE_RESERVED}},
		{TYPE_CFG0,     {"CfgRd",    "Reserved", "CfgWr",    "Reserved",
			TYPE_RESERVED}},
		{TYPE_CFG1,     {"CfgRd",    "Reserved", "CfgWr",    "Reserved",
			TYPE_RESERVED}},
		{TYPE_DMRW,     {"Reserved", "Reserved", "DMWr",     "DMWr",
		    TYPE_RESERVED}},
		{TYPE_CPL,      {"Cpl",      "Reserved", "CplD",     "Reserved",
			TYPE_RESERVED}},
		{TYPE_CPLLK,    {"CplLk",    "Reserved", "CplDLk",   "Reserved",
			TYPE_RESERVED}},
		{TYPE_FETCHADD, {"Reserved", "Reserved", "FetchAdd", "FetchAdd",
			TYPE_RESERVED}},
		{TYPE_SWAP,     {"Reserved", "Reserved", "Swap",     "Swap",
			TYPE_RESERVED}},
		{TYPE_CAS,      {"Reserved", "Reserved", "CAS",      "CAS",
			TYPE_RESERVED}},
		{TYPE_MSG0,     {"Reserved", "Msg " TYPE_MSG0_STR,      "Reserved",
			"MsgD " TYPE_MSG0_STR, TYPE_RESERVED}},
		{TYPE_MSG1,     {"Reserved", "Msg " TYPE_MSG1_STR,      "Reserved",
			"MsgD " TYPE_MSG1_STR, TYPE_RESERVED}},
		{TYPE_MSG2,     {"Reserved", "Msg " TYPE_MSG2_STR,      "Reserved",
			"MsgD " TYPE_MSG2_STR, TYPE_RESERVED}},
		{TYPE_MSG3,     {"Reserved", "Msg " TYPE_MSG3_STR,      "Reserved",
			"MsgD " TYPE_MSG3_STR, TYPE_RESERVED}},
		{TYPE_MSG4,     {"Reserved", "Msg " TYPE_MSG4_STR,      "Reserved",
			"MsgD " TYPE_MSG4_STR, TYPE_RESERVED}},
		{TYPE_MSG5,     {"Reserved", "Msg " TYPE_MSG5_STR,      "Reserved",
			"MsgD " TYPE_MSG5_STR, TYPE_RESERVED}},
	};

	for (int i = 0;
		i < (int)(sizeof(type_decode)/sizeof(type_decode[0]));
		i++) {
...
#define TLP_ADDRESS_64(tlp) (UINT64)(((UINT64)tlp->address_hi << 32) | \
										    (tlp->address_lo & 0xFFFC))
	...
	switch(req_tlp_header->type) {
		case TYPE_MR: // Memory
		case TYPE_IO: // IO
			struct mem_tlp_3dw *tlp_3dw = (struct mem_tlp_3dw *)tlp_header_log;
			struct mem_tlp_4dw *tlp_4dw = (struct mem_tlp_4dw *)tlp_header_log;
			...
			if (req_tlp_header->fmt == FMT_3DWND ||
				...
				snprintf(address, sizeof(address),
					", Address: 0x%08X", TLP_ADDRESS_32(tlp_3dw));
			} else {
				snprintf(address, sizeof(address),
					", Address: 0x%llX", TLP_ADDRESS_64(tlp_4dw));
			}
			break;
	...
	snprintf(desc, sizeof(desc),
		"TLP Header: %s, %s, TC=%d%s%s, Length=%d%s%s%s%s%s",
		fmt_str,
		type_str,
		req_tlp_header->tc,
		req_tlp_header->ep ? ", Poisoned" : "",
		req_tlp_header->td ? ", TLP Digest" : "",
		req_tlp_header->length,
		completer_id,
		requester_id,
		destination_id,
		completion_status,
		address);
	json_object_object_add(tlp_obj, "description", json_object_new_string(desc));
...
json_object *cper_section_pcie_to_ir(void *section)
{
	...
	char *encoded = NULL;
	struct capability_registers *cap_decode = NULL;
	if (isvalid_prop_to_ir(&ui64Type, 6)) {
		...
		cap_decode = (struct capability_registers *)&pcie_error->Capability
				     .PcieCap;
		...
		json_object_object_add(section_ir, "capabilityStructure", pcie_capability_ir);
	}
	...
	if (isvalid_prop_to_ir(&ui64Type, 7)) {
		...
		if (cap_decode &&
			cap_decode->pcie_capabilities_fields.cap_version >= 2 &&
			cap_decode->link_status_2_fields.flit_mode_status == 1) {
			...
			json_object_object_add(
				...
				parse_tlp_header_log(&aer_decode->tlp_header_0));
