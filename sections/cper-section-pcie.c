...
/* TODO: Verify boolean fields with the PCIe spec before adding them */
void add_json_field_bool(json_object *register_ir __attribute__((unused)),
			 const char *field_name __attribute__((unused)),
			 UINT64 value __attribute__((unused)))
{
...
json_object *pcie_aer_to_ir(EFI_PCIE_ERROR_DATA *pcie_error)
{
	...
	JSON_FIELD_INT(fields_ir, err_fatal_nonfatal_source_identification,
		       aer_decode->error_source_id);
