					     ...
					     char **desc_string)
{
	...
	*desc_string = NULL;
	if (size < sizeof(EFI_CXL_PROTOCOL_ERROR_DATA)) {
		cper_print_log("Error: CXL protocol section too small\n");
		return NULL;
	}
	...
	*desc_string = calloc(1, SECTION_DESC_STRING_SIZE);
	if (*desc_string == NULL) {
		cper_print_log(
			"Error: Failed to allocate CXL protocol desc string\n");
		return NULL;
	}
	...
	if (isvalid_prop_to_ir(&ui64Type, 1)) {
		...
		if (cxl_protocol_error->CxlAgentType ==
			...
			add_uint(agent_address, "functionNumber",
				 cxl_protocol_error->CxlAgentAddress
					 .DeviceAddress.FunctionNumber);
			add_uint(agent_address, "deviceNumber",
				 cxl_protocol_error->CxlAgentAddress
					 .DeviceAddress.DeviceNumber);
			add_uint(agent_address, "busNumber",
				 cxl_protocol_error->CxlAgentAddress
					 .DeviceAddress.BusNumber);
			add_uint(agent_address, "segmentNumber",
				 cxl_protocol_error->CxlAgentAddress
	...
	if (isvalid_prop_to_ir(&ui64Type, 3)) {
		...
		if (cxl_protocol_error->CxlAgentType ==
		    CXL_PROTOCOL_ERROR_DEVICE_AGENT) {
			add_uint(section_ir, "deviceSerial",
	...
	if (isvalid_prop_to_ir(&ui64Type, 5)) {
		...
		add_int(section_ir, "dvsecLength",
			cxl_protocol_error->CxlDvsecLength);
		//CXL DVSEC
	...
	if (isvalid_prop_to_ir(&ui64Type, 6)) {
		add_int(section_ir, "errorLogLength",
			cxl_protocol_error->CxlErrorLogLength);
