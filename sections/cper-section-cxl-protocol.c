/**
 * Describes functions for converting CXL protocol error CPER sections from
 *binary and JSON format into an intermediate format.
 *
...
json_object* cper_section_cxl_protocol_to_ir(void* section)
{
    ...
    if (cxl_protocol_error->CxlAgentType == CXL_PROTOCOL_ERROR_DEVICE_AGENT)
    {
        ...
        json_object_object_add(
            ...
            json_object_new_uint64(cxl_protocol_error->CxlAgentAddress
        ...
        json_object_object_add(
            ...
            json_object_new_uint64(
                cxl_protocol_error->CxlAgentAddress.DeviceAddress.BusNumber));
        json_object_object_add(
    ...
    if (cxl_protocol_error->CxlAgentType == CXL_PROTOCOL_ERROR_DEVICE_AGENT)
    {
        json_object_object_add(
            ...
            json_object_new_uint64(cxl_protocol_error->DeviceSerial));

        // The PCIe capability structure provided here could either be PCIe 1.1
        // Capability Structure (36-byte, padded to 60 bytes) or PCIe 2.0
        //Capability Structure (60-byte). There does not seem to be a way to
        // differentiate these, so this is left as a b64 dump.

        int32_t encoded_len = 0;

        encoded = base64_encode(
            (UINT8*)cxl_protocol_error->CapabilityStructure.PcieCap, 60,
            &encoded_len);
        if (encoded == NULL)
        {
    ...
    // CXL DVSEC
    // For CXL 1.1 devices, this is the "CXL DVSEC For Flex Bus Device"
    // structure as in CXL 1.1 spec. For CXL 1.1 host downstream ports, this is
    // the "CXL DVSEC For Flex Bus Port" structure as in CXL 1.1 spec.
    const char* cur_pos = (const char*)(cxl_protocol_error + 1);
    int32_t encoded_len = 0;

    encoded = base64_encode((UINT8*)cur_pos, cxl_protocol_error->CxlDvsecLength,
                            &encoded_len);
    if (encoded == NULL)
    {
        return NULL;
    ...
    encoded_len = 0;
    encoded = base64_encode(
        (UINT8*)cur_pos, cxl_protocol_error->CxlErrorLogLength, &encoded_len);
...
// Converts a single CXL protocol CPER-JSON section into CPER binary, outputting
// to the given stream.
void ir_section_cxl_protocol_to_cper(json_object* section, FILE* out)
{
    EFI_CXL_PROTOCOL_ERROR_DATA* section_cper = (EFI_CXL_PROTOCOL_ERROR_DATA*)
        calloc(1, sizeof(EFI_CXL_PROTOCOL_ERROR_DATA));

    // Validation bits.
    section_cper->ValidBits =
        ir_to_bitfield(json_object_object_get(section, "validationBits"), 7,
                       CXL_PROTOCOL_ERROR_VALID_BITFIELD_NAMES);

    // Detecting agent type.
    section_cper->CxlAgentType =
        readable_pair_to_integer(json_object_object_get(section, "agentType"));
    ...
    section_cper->DeviceId.VendorId =
        json_object_get_uint64(json_object_object_get(device_id, "vendorID"));
    section_cper->DeviceId.DeviceId =
        json_object_get_uint64(json_object_object_get(device_id, "deviceID"));
    ...
    section_cper->DeviceId.ClassCode =
        json_object_get_uint64(json_object_object_get(device_id, "classCode"));
    ...
    if (section_cper->CxlAgentType == CXL_PROTOCOL_ERROR_DEVICE_AGENT)
    {
        ...
        {
            memcpy(section_cper->CapabilityStructure.PcieCap, decoded,
                   decoded_len);
            free(decoded);
        }
    }
