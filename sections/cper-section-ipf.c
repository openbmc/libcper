				    ...
				    char **desc_string)
{
	...
	*desc_string = calloc(1, SECTION_DESC_STRING_SIZE);
	...
	add_bool(validation, "errorMapValid",
		 ipf_error->ValidBits.ProcErrorMapValid);
	add_bool(validation, "stateParameterValid",
		 ipf_error->ValidBits.ProcErrorMapValid);
	add_bool(validation, "crLIDValid", ipf_error->ValidBits.ProcCrLidValid);
	add_bool(validation, "psiStaticStructValid",
		 ipf_error->ValidBits.PsiStaticStructValid);
	add_bool(validation, "cpuInfoValid",
		 ipf_error->ValidBits.CpuIdInfoValid);
	...
	json_object *cache_errors = cper_ipf_mod_error_read_array(
		&cur_error, ipf_error->ValidBits.CacheCheckNum);
	json_object_object_add(section_ir, "cacheErrors", cache_errors);

	json_object *tlb_errors = cper_ipf_mod_error_read_array(
		&cur_error, ipf_error->ValidBits.TlbCheckNum);
	json_object_object_add(section_ir, "tlbErrors", tlb_errors);

	json_object *bus_errors = cper_ipf_mod_error_read_array(
		&cur_error, ipf_error->ValidBits.BusCheckNum);
	json_object_object_add(section_ir, "busErrors", bus_errors);

	json_object *reg_file_errors = cper_ipf_mod_error_read_array(
		&cur_error, ipf_error->ValidBits.RegFileCheckNum);
	json_object_object_add(section_ir, "regFileErrors", reg_file_errors);

	json_object *ms_errors = cper_ipf_mod_error_read_array(
		&cur_error, ipf_error->ValidBits.MsCheckNum);
	json_object_object_add(section_ir, "msErrors", ms_errors);
	...
	//BRs, CRs, ARs, RRs, FRs.
	json_object *brs = uint64_array_to_ir_array(psi_static->Brs, 8);
	json_object_object_add(psi_static_ir, "brs", brs);
	json_object *crs = uint64_array_to_ir_array(psi_static->Crs, 128);
	json_object_object_add(psi_static_ir, "crs", crs);
	json_object *ars = uint64_array_to_ir_array(psi_static->Ars, 128);
	json_object_object_add(psi_static_ir, "ars", ars);
	json_object *rrs = uint64_array_to_ir_array(psi_static->Rrs, 8);
	json_object_object_add(psi_static_ir, "rrs", rrs);
	json_object *frs = uint64_array_to_ir_array(psi_static->Frs, 256);
	json_object_object_add(psi_static_ir, "frs", frs);
