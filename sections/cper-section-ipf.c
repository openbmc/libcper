/**
 * Describes functions for converting Intel IPF CPER sections from binary and
...
json_object* cper_section_ipf_to_ir(void* section)
{
    ...
    json_object_object_add(
        validation, "errorMapValid",
        json_object_new_boolean(ipf_error->ValidBits.ProcErrorMapValid));
    json_object_object_add(
        validation, "stateParameterValid",
        json_object_new_boolean(ipf_error->ValidBits.ProcErrorMapValid));
    json_object_object_add(
    ...
    json_object_object_add(
        ...
        json_object_new_boolean(ipf_error->ValidBits.PsiStaticStructValid));
    json_object_object_add(
    ...
    // PSI validation bits.
    json_object* psi_validation = bitfield_to_ir(
        psi_static->ValidBits, 6, IPF_PSI_STATIC_INFO_VALID_BITFIELD_NAMES);
    json_object_object_add(psi_static_ir, "validationBits", psi_validation);

    // PSI minimal state save info.
    // stretch: structure min save state area as in Intel Itanium Architecture
...
// Reads a continuous stream of CPER IPF mod errors beginning from the given
// pointer, for n entries. Returns an array containing all read entries as JSON
// IR.
json_object* cper_ipf_mod_error_read_array(EFI_IPF_MOD_ERROR_INFO** cur_error,
                                           int num_to_read)
{
...
json_object* cper_ipf_mod_error_to_ir(EFI_IPF_MOD_ERROR_INFO* mod_error)
{
    ...
    json_object_object_add(mod_error_ir, "modRequestorID",
                           json_object_new_uint64(mod_error->ModRequestorId));
    json_object_object_add(mod_error_ir, "modResponderID",
