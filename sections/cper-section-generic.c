/**
 * Describes functions for converting processor-generic CPER sections from
...
json_object* cper_section_generic_to_ir(void* section)
{
    ...
    // Validation bits.
    json_object* validation = bitfield_to_ir(section_generic->ValidFields, 13,
    ...
    json_object* processor_type = integer_to_readable_pair(
        section_generic->Type, sizeof(GENERIC_PROC_TYPES_KEYS) / sizeof(int),
    ...
    json_object* processor_isa = integer_to_readable_pair(
        section_generic->Isa, sizeof(GENERIC_ISA_TYPES_KEYS) / sizeof(int),
        GENERIC_ISA_TYPES_KEYS, GENERIC_ISA_TYPES_VALUES, "Unknown (Reserved)");
    ...
    json_object* flags =
        bitfield_to_ir(section_generic->Flags, 4, GENERIC_FLAGS_BITFIELD_NAMES);
...
// Converts the given CPER-JSON processor-generic error section into CPER
...
void ir_section_generic_to_cper(json_object* section, FILE* out)
{
    ...
    section_cper->ValidFields =
        ir_to_bitfield(json_object_object_get(section, "validationBits"), 13,
                       GENERIC_VALIDATION_BITFIELD_NAMES);
    ...
    // Flags.
    section_cper->Flags =
        (UINT8)ir_to_bitfield(json_object_object_get(section, "flags"), 4,
                              GENERIC_FLAGS_BITFIELD_NAMES);

    // Various numeric/string fields.
    section_cper->Level =
        (UINT8)json_object_get_int(json_object_object_get(section, "level"));
    section_cper->VersionInfo = json_object_get_uint64(
        json_object_object_get(section, "cpuVersionInfo"));
    section_cper->ApicId =
        json_object_get_uint64(json_object_object_get(section, "processorID"));
