					...
					char **desc_string)
{
	...
	*desc_string = NULL;
	if (size < sizeof(EFI_PROCESSOR_GENERIC_ERROR_DATA)) {
		cper_print_log("Error: Generic Processor section too small\n");
		return NULL;
	}
	...
	*desc_string = calloc(1, SECTION_DESC_STRING_SIZE);
	if (*desc_string == NULL) {
		cper_print_log(
			"Error: Failed to allocate Generic Processor desc string\n");
		return NULL;
	}
	...
	if (isvalid_prop_to_ir(&ui64Type, 6)) {
		...
		add_uint(section_ir, "cpuVersionInfo",
			 section_generic->VersionInfo);
	}
	...
	if (isvalid_prop_to_ir(&ui64Type, 9)) {
		add_uint(section_ir, "targetAddress",
			 section_generic->TargetAddr);
	}

	if (isvalid_prop_to_ir(&ui64Type, 10)) {
		add_uint(section_ir, "requestorID",
			 section_generic->RequestorId);
	}

	if (isvalid_prop_to_ir(&ui64Type, 11)) {
		add_uint(section_ir, "responderID",
			 section_generic->ResponderId);
	}

	if (isvalid_prop_to_ir(&ui64Type, 12)) {
		add_uint(section_ir, "instructionIP",
			 section_generic->InstructionIP);
	}
