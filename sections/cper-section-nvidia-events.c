...
static size_t parse_cpu_info_to_bin(json_object *event_info_ir, FILE *out)
{
	...
	UINT32 major_rev =
		json_object_get_int(json_object_object_get(arch_ir, "majorRev"));
	UINT32 chip_id =
		json_object_get_int(json_object_object_get(arch_ir, "chipId"));
	UINT32 minor_rev =
		json_object_get_int(json_object_object_get(arch_ir, "minorRev"));
	...
	cpu_event_info.Architecture = (hid_fam & 0xF) |
				      ((major_rev & 0xF) << 4) |
				      ((chip_id & 0xFF) << 8) |
				      ((minor_rev & 0xF) << 16) |
				      ((pre_si & 0x1F) << 20) |
