	...
	do {                                                                           \
		if ((maj) != (exp_maj)) {                                              \
			cper_print_log(                                                \
				...
				(int)(exp_maj), (int)(maj), (int)(min),                \
				operation);                                            \
			break;                                                         \
		}                                                                      \
	} while (0)
// Check if info minor version is too old - if so, log error and break
#define NVIDIA_CHECK_INFO_MINOR_VERSION_MIN(maj, min, exp_maj, exp_min,                          \
					    operation)                                           \
	do {                                                                                     \
		if ((min) < (exp_min)) {                                                         \
			cper_print_log(                                                          \
				"Error: NVIDIA Event Info minor version too old: "               \
				"expected %d.%d or newer, got %d.%d. Skipping event info %s.\n", \
				(int)(exp_maj), (int)(exp_min), (int)(maj),                      \
				(int)(min), operation);                                          \
			break;                                                                   \
		}                                                                                \
	} while (0)
// Warn if info minor version is newer than expected (forward compatible)
#define NVIDIA_WARN_INFO_MINOR_VERSION_NEWER(maj, min, exp_maj, exp_min,                         \
					     operation)                                          \
	do {                                                                                     \
		if ((min) > (exp_min)) {                                                         \
			cper_print_log(                                                          \
				"Warning: NVIDIA Event Info minor version newer than expected: " \
...
#define NVIDIA_CHECK_EVENT_HEADER_VERSION(ver, exp_ver, operation, retval)      \
	do {                                                                    \
		if ((ver) != (exp_ver)) {                                       \
			cper_print_log(                                         \
				"Error: NVIDIA Event Header version mismatch: " \
					 ...
					 json_object *output_data_ir)
{
	...
	if (metadata->HardwareInfoType == 0) {
		...
		add_json_int_field(pci_info, JSON_GPU_METADATA_PCI_VENDOR_ID,
				   metadata->PciInfo.VendorId);
		add_json_int_field(pci_info, JSON_GPU_METADATA_PCI_DEVICE_ID,
					    ...
					    FILE *output_file_stream)
{
	...
	if (pci_info != NULL && metadata.HardwareInfoType == 0) {
		metadata.PciInfo.Class = get_json_int_field(
			pci_info, JSON_GPU_METADATA_PCI_CLASS);
					    ...
					    FILE *output_file_stream)
{
	...
	if (encoded_str == NULL) {
		cper_print_log(
			"Error: Failed to get string from 'data' field\n");
		return 0;
	}
	...
	*desc_string = malloc(SECTION_DESC_STRING_SIZE);
	...
	for (size_t i = 0; i < NVIDIA_EVENT_INFO_HANDLER_COUNT; i++) {
		if (NVIDIA_GET_EVENT_SOURCE_DEVICE_TYPE(event_header) ==
			...
			NVIDIA_CHECK_INFO_MAJOR_VERSION(
				info_major, info_minor,
				event_info_handlers[i].major_version,
				"parsing");
			NVIDIA_CHECK_INFO_MINOR_VERSION_MIN(
				info_major, info_minor,
				event_info_handlers[i].major_version,
				event_info_handlers[i].minor_version,
				"parsing");
			NVIDIA_WARN_INFO_MINOR_VERSION_NEWER(
				info_major, info_minor,
				event_info_handlers[i].major_version,
				event_info_handlers[i].minor_version,
				"parsing");
...
void ir_section_nvidia_events_to_cper(json_object *section, FILE *out)
{
	...
	for (size_t ctx_instance = 0; ctx_instance < ctx_count;
		...
		ctx.CtxSize = json_object_get_int(
			json_object_object_get(value, JSON_EVENT_CONTEXT_SIZE));
