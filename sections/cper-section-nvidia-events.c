...
static size_t parse_gpu_info_to_bin(json_object *event_info_ir, FILE *out)
{
	...
	gpu_event_info.EventOriginator = json_object_get_int64(
					      ...
					      char **desc_string)
{
	...
	*desc_string = malloc(SECTION_DESC_STRING_SIZE);
	...
	int outstr_len = 0;
	int sig_len = cper_printable_string_length(signature, 16);
	if (sig_len <= 0) {
		signature = "";
		sig_len = 0;
	}
	...
	json_object_object_add(
		event_header_ir, "contextCount",
		json_object_new_int64(event_header->EventContextCount));
	json_object_object_add(
		event_header_ir, "sourceDeviceType",
		json_object_new_int64(event_header->SourceDeviceType));
	...
	json_object_object_add(
		event_info_ir, "size",
		json_object_new_int64(event_info_header->InfoSize));
	...
	for (size_t i = 0;
	     i < sizeof(event_info_handlers) / sizeof(event_info_handlers[0]);
	     i++) {
	...
	for (size_t i = 0; i < (size_t)event_header->EventContextCount; i++) {
		...
		json_object_object_add(event_context_ir, "size",
				       json_object_new_int64(ctx->CtxSize));
...
void ir_section_nvidia_events_to_cper(json_object *section, FILE *out)
{
	...
	event_header.EventContextCount = json_object_get_int64(
		json_object_object_get(event_header_ir, "contextCount"));
	event_header.SourceDeviceType = json_object_get_int64(
		json_object_object_get(event_header_ir, "sourceDeviceType"));
	...
	event_info_header.InfoSize = json_object_get_int64(
		json_object_object_get(event_info_ir, "size"));
	...
	for (size_t ctx_instance = 0; ctx_instance < ctx_count;
		...
		ctx.CtxSize = json_object_get_int(
			json_object_object_get(value, "size"));
