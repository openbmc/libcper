				 ...
				 json_object *event_info_ir)
{
	...
	add_int(event_info_ir, "EventOriginator", gpu_event_info->EventOriginator); // UINT8
	add_int(event_info_ir, "SourcePartition", gpu_event_info->SourcePartition); // UINT16
	add_int(event_info_ir, "SourceSubPartition", gpu_event_info->SourceSubPartition); // UINT16
					 ...
					 json_object *output_data_ir)
{
	...
	if (metadata->HardwareInfoType == 0) {
		...
		add_int(pci_info, "subsystemVendorId", metadata->PciInfo.SubsystemVendorId);
					      ...
					      char **desc_string)
{
	...
	*desc_string = malloc(SECTION_DESC_STRING_SIZE);
	...
	for (size_t i = 0; i < (size_t)event_header->EventContextCount; i++) {
		...
		add_int(event_context_ir, "dataFormatType", ctx->DataFormatType);
		add_int(event_context_ir, "dataFormatVersion", ctx->DataFormatVersion);
