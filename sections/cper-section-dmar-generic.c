/**
 * Describes functions for converting generic DMAr CPER sections from binary and
...
json_object* cper_section_dmar_generic_to_ir(void* section)
{
    ...
    json_object_object_add(section_ir, "requesterID",
                           json_object_new_int(firmware_error->RequesterId));
    json_object_object_add(section_ir, "segmentNumber",
    ...
    json_object* access_type = integer_to_readable_pair(
        firmware_error->AccessType, 2, DMAR_GENERIC_ERROR_ACCESS_TYPES_KEYS,
    ...
    json_object* address_type = integer_to_readable_pair(
        firmware_error->AddressType, 2, DMAR_GENERIC_ERROR_ADDRESS_TYPES_KEYS,
    ...
    // Device address.
    json_object_object_add(section_ir, "deviceAddress",
                           json_object_new_uint64(firmware_error->DeviceAddr));

    return section_ir;
}

// Converts a single generic DMAR CPER-JSON section into CPER binary, outputting
// to the given stream.
void ir_section_dmar_generic_to_cper(json_object* section, FILE* out)
{
    EFI_DMAR_GENERIC_ERROR_DATA* section_cper = (EFI_DMAR_GENERIC_ERROR_DATA*)
        calloc(1, sizeof(EFI_DMAR_GENERIC_ERROR_DATA));
