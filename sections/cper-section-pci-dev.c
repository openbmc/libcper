					...
					char **desc_string)
{
	...
	*desc_string = calloc(1, SECTION_DESC_STRING_SIZE);
	...
	if (isvalid_prop_to_ir(&ui64Type, 1)) {
		...
		add_uint(id_info, "functionNumber",
			 dev_error->IdInfo.FunctionNumber);
		add_uint(id_info, "deviceNumber",
			 dev_error->IdInfo.DeviceNumber);
		add_uint(id_info, "busNumber", dev_error->IdInfo.BusNumber);
		add_uint(id_info, "segmentNumber",
	...
	if (isvalid_prop_to_ir(&ui64Type, 4)) {
		...
		for (int i = 0; i < num_data_pairs; i++) {
			...
			add_uint(register_data_pair, "secondHalf",
				 *(cur_pos + 1));
