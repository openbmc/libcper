/**
 * Describes functions for converting PCI/PCI-X device CPER sections from binary
 *and JSON format into an intermediate format.
 *
...
json_object* cper_section_pci_dev_to_ir(void* section)
{
    ...
    json_object_object_add(id_info, "vendorID",
                           json_object_new_uint64(dev_error->IdInfo.VendorId));
    json_object_object_add(id_info, "deviceID",
                           json_object_new_uint64(dev_error->IdInfo.DeviceId));
    json_object_object_add(id_info, "classCode",
    ...
    json_object_object_add(id_info, "busNumber",
    ...
    // The actual "pairs" of address and data aren't necessarily 8 bytes long,
    // so can't assume the contents. Hence the naming "firstHalf" and
    // "secondHalf" rather than "address" and "data".
...
void ir_section_pci_dev_to_cper(json_object* section, FILE* out)
{
    ...
    section_cper->ValidFields =
        ir_to_bitfield(json_object_object_get(section, "validationBits"), 5,
    ...
    section_cper->IdInfo.VendorId =
        json_object_get_uint64(json_object_object_get(id_info, "vendorID"));
    section_cper->IdInfo.DeviceId =
        json_object_get_uint64(json_object_object_get(id_info, "deviceID"));
    section_cper->IdInfo.ClassCode =
        json_object_get_uint64(json_object_object_get(id_info, "classCode"));
    section_cper->IdInfo.FunctionNumber = json_object_get_uint64(
        json_object_object_get(id_info, "functionNumber"));
    section_cper->IdInfo.DeviceNumber =
        json_object_get_uint64(json_object_object_get(id_info, "deviceNumber"));
    section_cper->IdInfo.BusNumber =
        json_object_get_uint64(json_object_object_get(id_info, "busNumber"));
