    ...
    cper_generic_error_status_to_ir(EFI_GENERIC_ERROR_STATUS* error_status)
{
    ...
    json_object_object_add(
        error_status_ir, "errorType",
        integer_to_readable_pair_with_desc(
            error_status->Type, 18, CPER_GENERIC_ERROR_TYPES_KEYS,
            CPER_GENERIC_ERROR_TYPES_VALUES,
            CPER_GENERIC_ERROR_TYPES_DESCRIPTIONS, "Unknown (Reserved)"));
    ...
    json_object_object_add(error_status_ir, "dataSignal",
    ...
    json_object_object_add(error_status_ir, "firstError",
...
// Converts a single uniform struct of UINT64s into intermediate JSON IR format,
// given names for each field in byte order.
...
// Converts a single uniform struct of UINT32s into intermediate JSON IR format,
// given names for each field in byte order.
...
// Converts a single integer value to an object containing a value, and a
...
// Converts a single integer value to an object containing a value, readable
// name and description if possible.
json_object* integer_to_readable_pair_with_desc(
    int value, int len, const int keys[], const char* values[],
                            ...
                            const char* names[])
{
    ...
    for (int i = 0; i < num_fields; i++)
    {
        json_object_object_add(result, names[i],
                               json_object_new_boolean((bitfield >> i) & 0x1));
    }

    return result;
}

// Converts the given IR bitfield into a standard UINT64 bitfield, with fields
// beginning from bit 0.
...
void string_to_timestamp(EFI_ERROR_TIME_STAMP* out, const char* timestamp)
{
    ...
    sscanf(timestamp, "%2hhu%2hhu-%hhu-%hhuT%hhu:%hhu:%hhu.000", &out->Century,
...
// Helper function to convert an EDK EFI GUID into a string for intermediate
// use.
void guid_to_string(char* out, EFI_GUID* guid)
{
    sprintf(out, "%08x-%04x-%04x-%02x%02x%02x%02x%02x%02x%02x%02x", guid->Data1,
...
void string_to_guid(EFI_GUID* out, const char* guid)
{
    ...
    sscanf(guid,
           ...
           &out->Data1, &out->Data2, &out->Data3, out->Data4, out->Data4 + 1,
           out->Data4 + 2, out->Data4 + 3, out->Data4 + 4, out->Data4 + 5,
           out->Data4 + 6, out->Data4 + 7);
}
