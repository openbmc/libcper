/**
 * Describes functions for generating pseudo-random specification compliant CPER
 *records.
 *
...
EFI_ERROR_SECTION_DESCRIPTOR* generate_section_descriptor(
    char* type, const size_t* lengths, int index, int num_sections);
size_t generate_section(void** location, char* type);

// Generates a CPER record with the given section types, outputting to the given
// stream.
void generate_cper_record(char** types, UINT16 num_sections, FILE* out)
{
    ...
    for (int i = 0; i < num_sections; i++)
    {
        ...
        if (section_lengths[i] == 0)
        {
            ...
            printf(
                "Error encountered generating section %d of type '%s', length returned zero.\n",
                i + 1, types[i]);
            return;
        }
    }
    ...
    for (int i = 0; i < num_sections; i++)
    {
        fwrite(section_descriptors[i], sizeof(EFI_ERROR_SECTION_DESCRIPTOR), 1,
               out);
        fflush(out);
...
void generate_single_section_record(char* type, FILE* out)
{
    ...
    section_descriptor->SectionOffset = sizeof(EFI_ERROR_SECTION_DESCRIPTOR);
...
// Generates a single section descriptor for a section with the given
// properties.
EFI_ERROR_SECTION_DESCRIPTOR* generate_section_descriptor(
    char* type, const size_t* lengths, int index, int num_sections)
{
    EFI_ERROR_SECTION_DESCRIPTOR* descriptor = (EFI_ERROR_SECTION_DESCRIPTOR*)
        generate_random_bytes(sizeof(EFI_ERROR_SECTION_DESCRIPTOR));
    ...
    // If section type is not "unknown", set section type GUID based on type
    // name.
    ...
    else
    {
        ...
        {
            if (strcmp(type, generator_definitions[i].ShortName) == 0)
            {
                memcpy(&descriptor->SectionType, generator_definitions[i].Guid,
                       sizeof(EFI_GUID));
                section_guid_found = 1;
                break;
            }
...
size_t generate_section(void** location, char* type)
{
    ...
    else
    {
        ...
        for (size_t i = 0; i < generator_definitions_len; i++)
        {
            if (strcmp(type, generator_definitions[i].ShortName) == 0)
            {
                length = generator_definitions[i].Generate(location);
                section_generated = 1;
                break;
            }
        }
    ...
    {
        printf(
            "Undefined section type '%s' given to generate. See 'cper-generate --help' for command information.\n",
            type);
        return 0;
    }
