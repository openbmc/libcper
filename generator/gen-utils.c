...
// Generates a random section of the given byte size, saving the result to the
// given location. Returns the length of the section as passed in.
...
// Creates a valid common CPER error section, given the start of the error
// section. Clears reserved bits.
void create_valid_error_section(UINT8* start)
{
    ...
    *(start + 1) =
        CPER_ERROR_TYPES_KEYS[rand() %
                              (sizeof(CPER_ERROR_TYPES_KEYS) / sizeof(int))];
}
