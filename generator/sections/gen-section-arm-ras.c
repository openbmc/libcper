					 ...
					 const UINT8 ipTypeFormatChoices[5])
{
	...
	// intentionall avoid soc specific identifier because it's not implemented
	...
	case 0:
		node->IPInstance.pe.MPIDR_EL1 = cper_rand();
		node->IPInstance.pe.Reserved = 0;
		break;
	case 1:
		node->IPInstance.systemPhysicalAddress.SystemPhysicalAddress =
			cper_rand();
		node->IPInstance.systemPhysicalAddress.Reserved = 0;
		break;
	case 2:
		node->IPInstance.localAddressIdentifier
			.SocSpecificLocalAddressSpace = cper_rand();
		node->IPInstance.localAddressIdentifier.BaseAddress =
			cper_rand();
		break;
	case 3:
		tmp = generate_random_bytes(16);
		if (!tmp) {
			return false;
		}
	...
	case 0:
		node->IPType.smmuIidr.MIDR_EL1 = cper_rand();
		node->IPType.smmuIidr.REVIDR_EL1 = cper_rand();
	...
	case 2:
		...
		node->IPType.pidr.PIDR0 = cper_rand();
		node->IPType.pidr.PIDR7 = cper_rand();
		node->IPType.pidr.PIDR6 = cper_rand();
		node->IPType.pidr.PIDR5 = cper_rand();
		node->IPType.pidr.PIDR4 = cper_rand();
		node->IPType.pidr.Reserved1 = 0;
		node->IPType.pidr.Reserved2 = 0;
		break;
	}

	generate_random_printable_string(node->UserData,
