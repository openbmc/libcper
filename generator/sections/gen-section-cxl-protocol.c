...
// Generates a single pseudo-random CXL protocol error section, saving the
// resulting address to the given location. Returns the size of the newly
// created section.
size_t generate_section_cxl_protocol(void** location)
{
    // Create a random length for the CXL DVSEC and CXL error log.
    // The logs attached here do not necessarily conform to the specification,
    // and are simply random.
    ...
    *validation &= 0x3F; // Validation bits 6-63.
    ...
    // We only reserve bytes if it's a CXL 1.1 device, and not a host downstream
    // port.
    if (cxl_agent_type == 0)
    {
        for (int i = 0; i < 3; i++)
    ...
    *reserved = 0;         // Reserved bytes 112-115.
    ...
    // If the device is a host downstream port, serial/capability structure is
    // invalid.
    if (cxl_agent_type != 0)
    {
        for (int i = 0; i < 68; i++)
