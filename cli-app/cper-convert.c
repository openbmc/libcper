/**
 * A user-space application linking to the CPER-JSON conversion library which
 *allows for easy conversion between CPER and CPER-JSON formats.
 *
...
int main(int argc, char* argv[])
{
    ...
    {
        printf(
            "Invalid number of arguments. See 'cper-convert --help' for command information.\n");
        return -1;
    }
    ...
    for (int i = 3; i < argc; i++)
    {
        ...
        else if (strcmp(argv[i], "--specification") == 0 && i < argc - 1)
        {
            // Specification file.
        ...
        {
            printf(
                "Unrecognised argument '%s'. See 'cper-convert --help' for command information.\n",
                argv[i]);
        }
    }
    ...
    {
        json_to_cper(input_file, output_file, specification_file, argv[0],
                     no_validate, debug);
    }
    else
    {
...
// Command for converting a provided CPER log file or CPER single section file
// into JSON.
void cper_to_json(char* in_file, char* out_file, int is_single_section)
{
    ...
    // Check whether there is a "--out" argument, if there is, then output to
    // file instead. Otherwise, just send to console.
    if (out_file == NULL)
    {
        printf("%s\n", json_output);
        return;
                  ...
                  char* program_dir, int no_validate, int debug)
{
    ...
    {
        printf(
            "No output file provided for 'to-cper'. See 'cper-convert --help' for command information.\n");
        return;
    }
    ...
    if (!no_validate)
    {
        ...
        if (specification_file == NULL)
        {
            ...
            // Make the specification path the assumed default (application
            // directory + specification/cper-json.json).
            specification_file = malloc(PATH_MAX);
            char* dir = dirname(program_dir);
            strcpy(specification_file, dir);
            strcat(specification_file, "/specification/cper-json.json");
        }
        ...
        int success =
            validate_schema_from_file(specification_file, ir, error_message);
...
void print_help(void)
{
    ...
    printf(
        "\tConverts the provided CPER log file into JSON, by default writing to stdout. If '--out' is specified,\n");
    printf(
        "\tThe outputted JSON will be written to the provided file name instead.\n");
    printf("\n:: to-json-section cper.section.file [--out file.name]\n");
    printf(
        "\tConverts the provided single CPER section descriptor & section file into JSON, by default writing to stdout.\n");
    ...
    printf(
        "\tConverts the provided CPER-JSON JSON file into CPER binary. An output file must be specified with '--out'.\n");
    printf(
        "\tWill automatically detect whether the JSON passed is a single section, or a whole file,\n");
    printf("\tand output binary accordingly.\n\n");
    printf(
        "\tBy default, the provided JSON will try to be validated against a specification. If no specification file path\n");
    printf(
        "\tis provided with '--specification', then it will default to 'argv[0] + /specification/cper-json.json'.\n");
    printf(
        "\tIf the '--no-validate' argument is set, then the provided JSON will not be validated. Be warned, this may cause\n");
    printf("\tpremature exit/unexpected behaviour in CPER output.\n\n");
    printf(
        "\tIf '--debug' is set, then debug output for JSON specification parsing will be printed to stdout.\n");
    printf("\n:: --help\n");
