/**
 * A very basic, non-complete implementation of a validator for the JSON Schema
 *specification, for validating CPER-JSON.
 *
...
// Validates a single JSON object against a provided schema file, returning 1 on
// success and 0 on failure to validate. Error message space must be allocated
// prior to call.
...
// Validates a single JSON object against a provided schema, returning 1 on
// success and 0 on failure to validate. Error message space must be allocated
// prior to call. If the schema does not include any other sub-schemas using
// "$ref", then leaving schema_directory as NULL is valid.
                    ...
                    json_object* object, char* error_message)
{
    ...
    if (schema_ver == NULL ||
        strcmp(json_object_get_string(schema_ver), JSON_SCHEMA_VERSION) != 0)
    {
        log_validator_error(
            error_message,
...
// Returns -1 on fatal/error failure, 0 on validation failure, and 1 on
// validation.
                   ...
                   json_object* object, char* error_message)
{
        ...
        json_object_get_type(ref_schema) == json_type_string)
    {
        ...
        log_validator_debug("loaded schema path '%s' for field '%s'.", ref_path,
                            field_name);
    }
    ...
    // If the schema contains a "oneOf" array, we need to validate the field
    // against each of the possible options in turn.
    ...
    if (one_of != NULL && json_object_get_type(one_of) == json_type_array)
    {
        ...
        for (int i = 0; i < len; i++)
        {
            ...
            if (one_of_option == NULL ||
                json_object_get_type(one_of_option) != json_type_object)
            {
                log_validator_debug(
            ...
            validated = validate_field(field_name, one_of_option, object,
                                       error_message);
            if (validated == -1)
            {
                return -1;
                    ...
                    json_object* object, char* error_message)
{
    ...
    {
        log_validator_debug("Required fields found for '%s', matching...",
                            field_name);
        ...
        json_object_get_type(properties) == json_type_object)
    {
        ...
        {
            // If the given property name does not exist on the target object,
            // ignore and continue next.
        ...
        // If additional properties are banned, validate that no additional
        // properties exist.
        if (!additional_properties_allowed)
        {
            json_object_object_foreach(object, key, value)
            {
                ...
                // If the given property name does not exist on the schema
                // object, fail validation.
...
int validate_array(const char* field_name, json_object* schema,
                   json_object* object, char* error_message)
{
    // Iterate all items in the array, and validate according to the "items"
    // schema.
...
// Logs an error message to the given error message location and (optionally)
