/**
 * Describes high level functions for converting an entire CPER log, and
 *functions for parsing CPER headers and section descriptions into an
...
json_object* cper_to_ir(FILE* cper_file)
{
    ...
    if (fread(&header, sizeof(EFI_COMMON_ERROR_RECORD_HEADER), 1, cper_file) !=
        1)
    {
    ...
    // Read the appropriate number of section descriptors & sections, and
    // convert them into IR format.
    ...
    for (int i = 0; i < header.SectionCount; i++)
    {
        ...
        json_object_array_add(
            sections_ir,
            cper_section_to_ir(cper_file, base_pos, &section_descriptor));
    }
...
json_object* cper_header_to_ir(EFI_COMMON_ERROR_RECORD_HEADER* header)
{
    ...
    json_object_object_add(
        error_severity, "name",
        json_object_new_string(severity_to_string(header->ErrorSeverity)));
    ...
    if (header->ValidationBits & 0x2)
    {
        ...
        json_object_object_add(header_ir, "timestamp",
    ...
    if (header->ValidationBits & 0x4)
    {
        ...
        json_object_object_add(header_ir, "partitionID",
    ...
    json_object_object_add(notification_type, "guid",
    ...
    json_object_object_add(notification_type, "type",
    ...
    json_object* flags = integer_to_readable_pair(
        header->Flags, sizeof(CPER_HEADER_FLAG_TYPES_KEYS) / sizeof(int),
        CPER_HEADER_FLAG_TYPES_KEYS, CPER_HEADER_FLAG_TYPES_VALUES, "Unknown");
    json_object_object_add(header_ir, "flags", flags);

    // Persistence information. Outside the scope of specification, so just a
    // uint32 here.
                                ...
                                EFI_ERROR_SECTION_DESCRIPTOR* descriptor)
{
    ...
    if (!section_converted)
    {
        ...
        char* encoded =
            base64_encode(section, descriptor->SectionLength, &encoded_len);
        if (encoded == NULL)
        {
...
// Converts a single CPER section, without a header but with a section
// descriptor, to JSON.
                                       ...
                                       size_t size)
{
    ...
    return cper_section_file ? cper_single_section_to_str_ir(cper_section_file)
                             : NULL;
}
