...
json_object *cper_buf_to_ir(const unsigned char *cper_buf, size_t size)
{
	...
	if (remaining < sizeof(EFI_COMMON_ERROR_RECORD_HEADER)) {
		cper_print_log("Invalid CPER file: Invalid header (incorrect signature).\n");
		goto fail;
	}
	...
	if (header->SectionCount == 0) {
		cper_print_log("Invalid CPER file: Invalid section count (0).\n");
		goto fail;
	}
	...
	for (int i = 0; i < header->SectionCount; i++) {
		...
		if (remaining < sizeof(EFI_ERROR_SECTION_DESCRIPTOR)) {
			cper_print_log("Invalid number of section headers: Header states %d sections, could not read section %d.\n",
		...
		if (section_descriptor->SectionOffset > size) {
			cper_print_log("Invalid section descriptor: Section offset > size.\n");
			goto fail;
		}

		if (section_descriptor->SectionLength <= 0) {
			cper_print_log("Invalid section descriptor: Section length <= 0.\n");
			goto fail;
		}

		if (section_descriptor->SectionOffset >
		    UINT_MAX - section_descriptor->SectionLength) {
			cper_print_log("Invalid section descriptor: Section offset + length would overflow.\n");
			goto fail;
		}
		...
		if (section_descriptor->SectionOffset +
		    ...
		    size) {
			cper_print_log("Invalid section descriptor: Section offset + length > size.\n");
			goto fail;
		}
...
json_object *cper_to_ir(FILE *cper_file)
{
	...
	if (fread(&header, sizeof(EFI_COMMON_ERROR_RECORD_HEADER), 1,
		  cper_file) != 1) {
		cper_print_log("Invalid CPER file: Invalid length (log too short).\n");
		return NULL;
	}
					   ...
					   size_t size)
{
	...
	if (sizeof(EFI_ERROR_SECTION_DESCRIPTOR) > size) {
		cper_print_log("Size of cper buffer was too small to read section descriptor %zu\n",
		       size);
		return NULL;
	}
...
json_object *cper_single_section_to_ir(FILE *cper_section_file)
{
	...
	if (fread(&section_descriptor, sizeof(EFI_ERROR_SECTION_DESCRIPTOR), 1,
		  cper_section_file) != 1) {
		cper_print_log("Failed to read section descriptor for CPER single section (fread() returned an unexpected value).\n");
		json_object_put(ir);
		return NULL;
	}
	...
	if (fread(section, section_descriptor.SectionLength, 1,
		  cper_section_file) != 1) {
		cper_print_log("Section read failed: Could not read %u bytes from global offset %d.\n",
		       section_descriptor.SectionLength,
