 ...
 */
typedef struct {
	UINT32 receiver_error_masked : 1;	    // bit [0]
	UINT32 rsvdz1 : 5;			    // bits [5:1]
	UINT32 bad_tlp_masked : 1;		    // bit [6]
	UINT32 bad_dllp_masked : 1;		    // bit [7]
	UINT32 replay_num_rollover_masked : 1;	    // bit [8]
	UINT32 rsvdz2 : 3;			    // bits [11:9]
	UINT32 replay_timer_timeout_masked : 1;	    // bit [12]
	UINT32 advisory_non_fatal_error_masked : 1; // bit [13]
	UINT32 corrected_internal_error_masked : 1; // bit [14]
	UINT32 header_log_overflow_masked : 1;	    // bit [15]
	UINT32 rsvdz3 : 16;			    // bits [31:16]
