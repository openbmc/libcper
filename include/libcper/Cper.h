...
typedef struct {
	...
	UINT8 Reserved0[3];  /* Reserved, must be 0 */
	UINT32 ErrorSyndromeArrayNumEntries; /* N: number of descriptors (Table 21) */
	UINT16 ErrorSyndromeArrayOffset; /* err_arr_off: offset to error syndrome array (from start of section) */
	UINT16 AuxiliaryDataOffset; /* aux_off: offset to auxiliary data block (0 if absent) */
	UINT8 IPInstanceFormat; /* IP_instance_format (0..3) */
	UINT8 IPTypeFormat;	/* IP_type_format (0..3, 255 invalid) */
	UINT8 Reserved1[6];	/* Reserved, must be 0 */
	UINT8 IPInstance[16]; /* 16 bytes; interpretation per IPInstanceFormat */
	UINT8 IPType[24];     /* 24 bytes; interpretation per IPTypeFormat */
	CHAR8 UserData[16]; /* Null-terminated implementation-defined string */
} __attribute__((packed, aligned(1))) EFI_ARM_RAS_NODE;
...
} __attribute__((packed, aligned(1))) EFI_ARM_RAS_ERROR_RECORD_DESCRIPTOR;
...
} __attribute__((packed, aligned(1))) EFI_ARM_RAS_AUX_DATA_HEADER;
...
} __attribute__((packed, aligned(1))) EFI_ARM_RAS_AUX_CONTEXT_HEADER;
