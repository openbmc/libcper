...
// Converts the given JSON IR CPER representation into CPER binary format, piped
// to the provided file stream. This function performs no validation of the IR
// against the CPER-JSON specification. To ensure a safe call, use
// validate_schema() from json-schema.h before attempting to call this function.
void ir_to_cper(json_object* ir, FILE* out)
{
    // Create the CPER header.
    EFI_COMMON_ERROR_RECORD_HEADER* header = (EFI_COMMON_ERROR_RECORD_HEADER*)
        calloc(1, sizeof(EFI_COMMON_ERROR_RECORD_HEADER));
    ...
    for (int i = 0; i < amt_descriptors; i++)
    {
        ...
        ir_section_descriptor_to_cper(
            json_object_array_get_idx(section_descriptors, i), descriptors[i]);
                       ...
                       EFI_COMMON_ERROR_RECORD_HEADER* header)
{
    ...
    // Section count.
    int section_count =
        json_object_get_int(json_object_object_get(header_ir, "sectionCount"));
    ...
    header->ValidationBits =
        ir_to_bitfield(json_object_object_get(header_ir, "validationBits"), 3,
    ...
    if (timestamp != NULL)
    {
        ...
        header->TimeStamp.Flag = json_object_get_boolean(
            json_object_object_get(header_ir, "timestampIsPrecise"));
    }
    ...
    string_to_guid(
        &header->CreatorID,
        json_object_get_string(json_object_object_get(header_ir, "creatorID")));
    ...
    string_to_guid(&header->NotificationType,
                   json_object_get_string(
                       json_object_object_get(notification_type, "guid")));

    // Record ID, persistence info.
    header->RecordID =
        json_object_get_uint64(json_object_object_get(header_ir, "recordID"));
    ...
    header->Flags =
        (UINT32)json_object_get_uint64(json_object_object_get(flags, "value"));
}
                        ...
                        EFI_ERROR_SECTION_DESCRIPTOR* descriptor, FILE* out)
{
    ...
    {
        if (guid_equal(section_definitions[i].Guid, &descriptor->SectionType) &&
                                   ...
                                   EFI_ERROR_SECTION_DESCRIPTOR* descriptor)
{
    ...
    descriptor->SecValidMask = ir_to_bitfield(
        json_object_object_get(section_descriptor_ir, "validationBits"), 2,
        CPER_SECTION_DESCRIPTOR_VALID_BITFIELD_NAMES);
    descriptor->SectionFlags =
        ir_to_bitfield(json_object_object_get(section_descriptor_ir, "flags"),
    ...
    string_to_guid(
        ...
        json_object_get_string(json_object_object_get(section_type, "data")));
...
void ir_single_section_to_cper(json_object* ir, FILE* out)
{
    ...
    EFI_ERROR_SECTION_DESCRIPTOR* section_descriptor =
        (EFI_ERROR_SECTION_DESCRIPTOR*)calloc(
            1, sizeof(EFI_ERROR_SECTION_DESCRIPTOR));
    ir_section_descriptor_to_cper(
        json_object_object_get(ir, "sectionDescriptor"), section_descriptor);
    fwrite(section_descriptor, sizeof(EFI_ERROR_SECTION_DESCRIPTOR), 1, out);
    fflush(out);
